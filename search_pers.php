...
$query_rsPersonnes = sprintf("SELECT personnes.personne_id, numero_compte, cle, personnes.personne_matricule, personne_nom, personne_prenom, add_commission, display
							 FROM personnes, rib WHERE personnes.personne_id = rib.personne_id AND personne_nom LIKE %s AND display = '1' ORDER BY personne_nom ASC", GetSQLValueString("%" . $colname_rsPersonnes . "%", "text"));
} else {
$query_rsPersonnes = sprintf("SELECT personnes.personne_id, numero_compte, cle, personnes.personne_matricule, personne_nom, personne_prenom, add_commission, display 
FROM personnes, rib 
...
<h1>Rechercher une personne dans le fichier</h1>
<table width="60%" border="1" align="center" class="std">
  ...
  <tr>
    ...
    <?php } else { ?>
    <th nowrap="nowrap">Rechercher</th>
    <?php } ?>
  ...
  <tr bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
	...
	<?php if (isset($_GET['acID']) && $_GET['acID']=='add') { ?>
      <a href="#" onclick="window.opener.location.href='sample125.php?acID=add&ovID=<?php echo $_GET['ovID']; ?>&perID=<?php echo $row_rsPersonnes['personne_id']; ?>';self.close();"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/></a>
      <?php } else {  ?> 
      <a href="#" onclick="window.opener.location.href='sample125.php?comID=<?php echo $_GET['comID']; ?>&perID=<?php echo $row_rsPersonnes['personne_id']; ?>';self.close();"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/></a>     
      <?php } ?>&nbsp; </td>
  </tr>
    ...
    <td colspan="4"><table width="100%" border="1" align="left">
      ...
      <?php if(isset($_POST['button'])) {?>
        ...
        <?php           
	 	  ...
	 	  ?>  
        <a href="#" onclick="<?php //header(sprintf("Location: %s", $showGoTo)); ?>"></BR>Ajouter une nouvelle personne<img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle" /></a></td>
      </tr>
    </table></td>
  </tr>
  <?php } ?>
