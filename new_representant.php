...
$x = 0; do { $x++;
$verifyRepresentant = (MinmapDB::getInstance()->verify_commissions_representant($_POST['commission_id'], $_POST['personne_id'.$x]));
if ($verifyRepresentant){
$updateSQL = sprintf("UPDATE membres SET display='1' WHERE commissions_commission_id = %s AND personnes_personne_id=%s",
                       GetSQLValueString($_POST['commission_id'], "int"), GetSQLValueString($_POST['personne_id'.$x], "int"));

$updateSQL1 = sprintf("UPDATE personnes SET add_commission='0' WHERE personne_id=%s",
...
$insertSQL = sprintf("INSERT INTO membres (commissions_commission_id, fonctions_fonction_id, personnes_personne_id, montant, checboxName, position, dateCreation) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
$updateSQL = sprintf("UPDATE personnes SET add_commission='0' WHERE personne_id=%s",
                       GetSQLValueString($_POST['personne_id'.$x], "int"));
...
<table width="60%" border="1" align="center" class="std">
  ...
  <?php $counter=0; do  { $counter++; ?>
    <tr bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
      ...
      <td><?php echo $row_rsPersonnes['numero_compte']."-".$row_rsPersonnes['cle']; ?>&nbsp; </td>
      ...
      <td align="center"><?php if (isset($row_rsPersonnes['add_commission'])) { ?>
          ...
          <a href="change_etat_index.php?comID=<?php echo $_GET['comID']; ?>&recordID=<?php echo $row_rsPersonnes['personne_id']; ?>&map=personnes&amp;mapid=personne_id&amp;action=add_com&page=rep"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/></a>
          <?php } else {?>
          <a href="change_etat_index.php?recordID=<?php echo $row_rsPersonnes['personne_id']; ?>&amp;map=personnes&amp;&amp;mapid=personne_id&amp;action=add_com&page=rep"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/></a>
          <?php } ?>
        <?php }  ?>
  &nbsp;
    </td>
    ...
    <td colspan="5"><table width="100%" border="1" align="left">
      ...
      <tr>
        <td>Ajouter une personne
          <a href="new_personnes.php?menuID=2&amp;action=new&page=new_representant&comID=<?php echo $_GET['comID']; ?>&menuID=<?php echo $_GET['menuID']; ?>"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle" /></a></td>
      </tr>
    </table></td>
  </tr>
<?php } ?>
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <tr>
    <td colspan="8">Ajouter une commission
      <?php           
	$showGoToPersonne = "search_commissions.php?page=rep";
/*  if (isset($_SERVER['QUERY_STRING'])) {
...
<?php
            ...
            if ($commissionIsEmpty && empty($_GET['comID'])) { ?>
<div class="control"><img src="images/img/s_error.png" alt="" width="16" height="16" align="absmiddle" />Choisir la commission dépendante, SVP</div>
<?php }
            ?>
&nbsp;
<?php
...
<?php }
  ...
  <tr align="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
    ...
    <td><?php echo $row_rsPersonnesAdd['personne_nom'] . " " . $row_rsPersonnesAdd['personne_prenom']; ?>&nbsp; </td>
    <td><input name="textfield" type="text" id="textfield" value="Représentant Maitre d'ouvrage" size="32" readonly="readonly" />&nbsp;
      <input type="hidden" name="<?php echo "fonction_id" . $counter ?>" id="<?php echo "fonction_id" . $counter ?>" value="40" align="right"/>
      &nbsp;</td>
    <td>
      <?php if (isset ($_GET['comID'])) { ?> 
      <a href="change_etat_index.php?comID=<?php echo $_GET['comID'];?>&&recordID=<?php echo $row_rsPersonnesAdd['personne_id']; ?>&amp;map=personnes&amp;mapid=personne_id&amp;action=move_com&page=rep"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle" /></a>
      <?php } else { ?>
      <a href="change_etat_index.php?recordID=<?php echo $row_rsPersonnesAdd['personne_id']; ?>&amp;map=personnes&amp;mapid=personne_id&amp;action=move_com&page=rep"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle" /></a>   
      <?php } ?>
  </tr>
...
mysql_free_result($rsSelStructure);
?>
