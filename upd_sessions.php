...
/*echo "Champ_commissions =" . $_POST['membres_commissions_commission_id']. "Champ_fonctions =" .$_POST['membres_fonctions_fonction_id'.$x]. "Champ_dossiers =" .$_POST['dossiers_dossier_id']. "Champ_nbr_dossier =" .$_POST['nombre_dossier']. "Champ_jour =" .count($_POST['jour'.$x]). "Champ_variable =" .implode( "**", $_POST['jour'.$x] ). "Champ_mois =" .$_POST['mois']. "Champ_annee =" .$_POST['annee']. "Champ_date =" .$date. "Champ_display =" .$_POST['display']. "Champ_personne =" .$_POST['membres_personnes_personne_id'.$x]. "Champ_mois2 =" .$_POST['mois2']. "Champ_annee2 =" .$_POST['annee2'].'</BR>';*/
// debut boucle
...
$updateSQL = sprintf("UPDATE sessions SET membres_fonctions_fonction_id=%s, dossiers_dossier_id=%s, nombre_dossier=%s, Nombre_jour=%s, jour=%s, mois=%s, annee=%s, dateUpdate=%s, display=%s WHERE membres_commissions_commission_id=%s AND membres_personnes_personne_id=%s AND mois = %s AND annee = %s",
...
AND sessions.membres_fonctions_fonction_id <> 41
...
AND sessions.membres_fonctions_fonction_id <> 41
AND commissions.commission_parent = %s 
AND commissions.commission_id = %s 
AND mois = %s 
AND annee = %s 
ORDER BY fonctions.fonction_id", GetSQLValueString($colname_rsSessions3, "int"),GetSQLValueString($colname_rsSessions, "int"),GetSQLValueString($colname_rsSessions1, "text"),GetSQLValueString($colname_rsSessions2, "text")); //}*/
...
<table width="75%" border="1" align="left" class="std2">
    ...
    <tr>
      <td><table width="75%" border="1" class="std2">
        ...
        &nbsp;</td>
          <th align="right" nowrap="nowrap">Nombres de dossiers : </th>
          <td nowrap="nowrap"><input type="text" name="nombre_dossier" value="<?php echo htmlentities($row_rsUpdSessions['nombre_dossier'], ENT_COMPAT, 'utf-8'); ?>" size="32" /></td>
        </tr>
        <tr valign="baseline">
          <th height="58" align="right" valign="top" nowrap="nowrap">Ref Dossier :</th>
          <td colspan="5" valign="top"><textarea name="dossiers_dossier_id" id="dossiers_dossier_id" cols="100" rows="3"><?php echo htmlentities($row_rsUpdSessions['dossiers_dossier_id'], ENT_COMPAT, 'utf-8'); ?></textarea>
<?php           
