  ...
  <tr>
	...
	<tr>
      ...
      <tr>
		...
		?>&nbsp; </a></td>
        <td align="left" nowrap="nowrap"><?php echo $row_rsSessions['fonction_lib']; ?>
		...
		mysql_select_db($database_MyFileConnect, $MyFileConnect);
		$query_DetailRS1 = sprintf("SELECT fonction_lib FROM fonctions  WHERE fonction_lib = %s", GetSQLValueString($colname_DetailRS1, "text"));


	    mysql_select_db($database_MyFileConnect, $MyFileConnect);
		$query_rsFonctions = sprintf("SELECT count(fonction_id) as total, fonction_lib, montant, membres_commissions_commission_id  
		FROM sessions, fonctions, membres
		WHERE membres_fonctions_fonction_id = fonction_id
		AND fonctions_fonction_id = fonction_id
		AND personnes_personne_id = membres_personnes_personne_id
		AND membres_personnes_personne_id = %s
		AND sessions.mois between %s AND %s 
		AND sessions.annee = %s 
		AND sessions.membres_commissions_commission_id = %s 
		GROUP BY fonction_id",
		GetSQLValueString($row_rsSessions['personne_id'], "text"),
		GetSQLValueString($txtMonth1, "text"),
		GetSQLValueString($txtMonth2, "text"), 
		GetSQLValueString($txtYear, "text"), 
		GetSQLValueString($txtComID, "int"));
        ...
        <table border="1" align="center">
          ...
          <?php do { ?>
            <tr>
              <td><?php echo $row_rsFonctions['total']; ?>&nbsp; </td>
              <td><?php echo $row_rsFonctions['fonction_lib']; ?>&nbsp; </td>
              <td><?php echo $row_rsFonctions['montant']; ?>&nbsp; </td>
            </tr>
            <?php } while ($row_rsFonctions = mysql_fetch_assoc($rsFonctions)); ?>
        </table>&nbsp;</td>
