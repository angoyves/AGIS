...
$updateSQL = sprintf("UPDATE sessions SET membres_fonctions_fonction_id=%s, dossiers_dossier_id=%s, nombre_dossier=%s, Nombre_jour=%s, jour=%s, mois=%s, annee=%s, dateUpdate=%s, display=%s WHERE membres_commissions_commission_id=%s AND membres_personnes_personne_id=%s AND mois = %s AND annee = %s",
                       ...
                       GetSQLValueString($_GET['sCom'], "int"),
...
AND (sessions.membres_fonctions_fonction_id BETWEEN 1 AND 5 OR sessions.membres_fonctions_fonction_id = 141)
AND commissions.commission_id = %s 
AND mois = %s 
AND annee = %s 
ORDER BY commission_id, fonctions.fonction_id", GetSQLValueString($colname_rsSessions3, "int"),GetSQLValueString($colname_rsSessions1, "text"),GetSQLValueString($colname_rsSessions2, "text"));
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table width="75%" border="0" align="center">
    ...
    <tr>
      <td><h1>Sous Commissions dependantes</h1>&nbsp;</td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    </tr>
...
// debut
 foreach(GetValueList($row_rsJourMois['mois_id']) as $value){ ?>
	...
	?>
          <td nowrap="nowrap"><a href="<?php echo $link5 . $row_rsSessions['personne_id']; ?>" onclick="return confirm('Etes vous sur de vouloir supprimer 																																																				cette sessions ?');"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
          </tr>
