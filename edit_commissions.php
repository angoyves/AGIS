...
$colname_rsUpdCommission = "-1";
if (isset($_GET['comID'])) {
  $colname_rsUpdCommission = $_GET['comID'];
}
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsUpdCommission = sprintf("SELECT * FROM commissions WHERE commission_id = %s", GetSQLValueString($colname_rsUpdCommission, "int"));
$rsUpdCommission = mysql_query($query_rsUpdCommission, $MyFileConnect) or die(mysql_error());
$row_rsUpdCommission = mysql_fetch_assoc($rsUpdCommission);
$totalRows_rsUpdCommission = mysql_num_rows($rsUpdCommission);

mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsLocalite = "SELECT localite_id, localite_lib FROM localites WHERE display = '1'";
...
do {  
?>
        <option value="<?php echo $row_rsLocalite['localite_id']?>"<?php if (!(strcmp($row_rsLocalite['localite_id'], htmlentities($row_rsUpdCommission['localite_id'], ENT_COMPAT, 'utf-8')))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsLocalite['localite_lib']?></option>
        <?php
...
do {  
?>
        <option value="<?php echo $row_rsTypeCommission['type_commission_id']?>"<?php if (!(strcmp($row_rsTypeCommission['type_commission_id'], htmlentities($row_rsUpdCommission['type_commission_id'], ENT_COMPAT, 'utf-8')))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsTypeCommission['type_commission_lib']?></option>
        <?php
...
do {  
?>
        <option value="<?php echo $row_rsNature['nature_id']?>"<?php if (!(strcmp($row_rsNature['nature_id'], htmlentities($row_rsUpdCommission['nature_id'], ENT_COMPAT, 'utf-8')))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsNature['lib_nature']?></option>
        <?php
...
?>
    ...
    <tr> </tr>
    <tr valign="baseline">
      <th nowrap="nowrap" align="right">Commission :</th>
      <td><input type="text" name="commission_lib" value="<?php echo htmlentities($row_rsUpdCommission['commission_lib'], ENT_COMPAT, 'utf-8'); ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
...
do {  
?>
        <option value="<?php echo $row_rsUpdCommission['commission_parent']?>" <?php if (!(strcmp($row_rsUpdCommission['commission_parent'], htmlentities($row_rsUpdCommission['commission_parent'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>><?php echo $row_rsUpdCommission['commission_parent']?></option>
        <?php
} while ($row_rsUpdCommission = mysql_fetch_assoc($rsUpdCommission));
?>
      </select></td>
    </tr>
    <tr> </tr>
  ...
  <input type="hidden" name="commission_id" value="<?php echo $row_rsUpdCommission['commission_id']; ?>" />
  <input type="hidden" name="membre_insert" value="<?php echo htmlentities($row_rsUpdCommission['membre_insert'], ENT_COMPAT, 'utf-8'); ?>" />
  <input type="hidden" name="dateCreation" value="<?php echo htmlentities($row_rsUpdCommission['dateCreation'], ENT_COMPAT, 'utf-8'); ?>" />
  <input type="hidden" name="dateUpdate" value="<?php echo htmlentities($row_rsUpdCommission['dateUpdate'], ENT_COMPAT, 'utf-8'); ?>" />
  <input type="hidden" name="display" value="<?php echo htmlentities($row_rsUpdCommission['display'], ENT_COMPAT, 'utf-8'); ?>" />
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsUpdCommission);
