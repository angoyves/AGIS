...
$txtTaux = (MinmapDB::getInstance()->get_taux($txtYear)); 
...
if (trim($txtYear) == '')
	  ...
	  mysql_select_db($database_MyFileConnect, $MyFileConnect);
		$query_rsSessions = sprintf("SELECT commission_id, commission_lib, type_commission_id, fonctions.fonction_id, personnes.personne_id, personne_nom, personne_prenom, fonction_lib, personne_telephone, nombre_jour, jour, mois, annee, montant, (nombre_jour * montant) as total
		...
		AND ((membres.fonctions_fonction_id BETWEEN 1 AND 40) OR membres.fonctions_fonction_id = 141) 
		...
		ORDER BY sessions.membres_commissions_commission_id, fonctions.fonction_id, personne_nom",GetSQLValueString($txtMonth1, "text"),GetSQLValueString($txtMonth2, "text"), GetSQLValueString($txtYear, "text"), GetSQLValueString($txtComID, "int"), GetSQLValueString($txtComID, "int"));
...
<table width="60%" border="0" align="center">
  ...
  <tr>
    <td colspan="3" align="center" valign="middle">ETAT DES COMMISIONS DE PASSATION DES MARCHES</td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<tr>
        ...
        <th nowrap="nowrap">Net à payer<?php echo ((isset($txtTaux) && $txtTaux>0 && $txtTaux<100)?' ('.$txtTaux.'%)':''); ?></th>
	  ...
	  do  { $compter++; ?>
      <?php           
            $showGoToPersonnes3 = "upd_rib.php?recordID=". $row_rsSessions['personne_id'];
      ...
      ?>
		...
		?>&nbsp; </a></td>
        <td align="center" nowrap="nowrap"><?php echo $row_rsSessions['fonction_lib']; ?>
        &nbsp;</td>
        <td align="center" nowrap="nowrap"><a href="#" onclick="<?php popup($showGoToPersonnes1, "700", "400"); ?>">
          <?php $user_rib = MinmapDB::getInstance()->get_user_rib_by_id($row_rsSessions['personne_id']);
		if (isset($user_rib) && $user_rib <> 'xxxxxxxxxxxxxxxxxxxxxxx'){
		  echo $user_rib; }else{ echo 'Absent'; } ?>
        </a></td>
        <td align="left" valign="middle"><?php echo $row_rsSessions['personne_telephone']; ?></td>
		...
		($montantRetenu < 0 ? $montantRetenu = 0 : $montantRetenu = ($resteAPaye*$_SESSION['MM_Retenu'])*$txtTaux/100);
		$somMontantRetenu = $somMontantRetenu + $montantRetenu;
		$netAPercevoir = ($resteAPaye - $montantRetenu)*$txtTaux/100; 
...
<tr>
        <td colspan="5" nowrap="nowrap"> <a href="#" onclick="<?php popup($showGoToPersonnes1, "700", "400"); ?>">&nbsp; </a> </td>
  ...
  <tr>
    <td colspan="3" align="left"><?php echo '<img src="'.$PNG_WEB_DIR.basename($filename).'" /><hr/>'; ?><strong><?php echo $codeEtatEdition. '- Etat Edité par :' . MinmapDB::getInstance()->get_user_name($_SESSION['MM_UserID']); ?></strong></td>
  </tr>
</table>
</body>
</html>
