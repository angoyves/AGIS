...
$query_rsSessionsCommite = sprintf("SELECT commission_id, commission_lib, personne_id, personne_nom, fonction_lib, nombre_jour, jour, mois, annee, montant, (nombre_jour * montant) as total
...
ORDER BY annee, mois, fonctions.fonction_id ", GetSQLValueString($colname2_rsSessionsCommite, "text"),GetSQLValueString($colname3_rsSessionsCommite, "text"),GetSQLValueString($colname4_rsSessionsCommite, "text"), GetSQLValueString($colname6_rsSousCommission, "int"));
  ...
  <table border="1" align="center" class="std">
    ...
    <?php $compter = 0; do { $compter++; ?>
      <tr>
        ...
        <td nowrap="nowrap">&nbsp; <?php echo htmlentities(strtoupper($row_rsSessionsCommite['personne_nom'] . " " . $row_rsSessionsCommite['personne_prenom'])); ?></td>
        <td nowrap="nowrap"><?php echo $row_rsSessionsCommite['fonction_lib']; ?>&nbsp;</td>
        <td nowrap="nowrap">&nbsp;</td>
        <?php $counter = $_POST['txtMonth1'];  $total_montant = 0; do {   GetValueMonth($counter);  ?>
        <td align="right" nowrap="nowrap">
		<?php 
		$nombre_jours = MinmapDB::getInstance()->get_nombre_jour_by_values($_GET[comID], $row_rsSessionsCommite['personne_id'], $counter, $_POST['txtYear']);
			...
			?>&nbsp;</td>
      <?php $counter++;} while ($counter <= $_POST['txtMonth2']) ?>
      <td align="right" nowrap="nowrap"><?php echo number_format($row_rsSessionsCommite['montant'],0,' ',' '); ?></td>
      <td align="right" nowrap="nowrap"><?php echo number_format($row_rsSessionsCommite['total'],0,' ',' '); $somme = $somme + $row_rsSessionsCommite['total']; ?>&nbsp;</td>
      </tr>
