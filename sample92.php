...
AND annee = %s 
AND (mois = %s OR mois = %s OR mois = %s OR mois = %s OR mois = %s 
	OR mois = %s OR mois = %s OR mois = %s OR mois = %s OR mois = %s
	OR mois = %s OR mois = %s) 
GROUP BY personnes.personne_id
ORDER BY fonctions.fonction_id", 
GetSQLValueString($colname_rsSessions, "int"),
GetSQLValueString($colname_rsSessions2, "text"),
GetSQLValueString($_POST['checkbox1'], "text"),
GetSQLValueString($_POST['checkbox2'], "text"), 
GetSQLValueString($_POST['checkbox3'], "text"),
GetSQLValueString($_POST['checkbox4'], "text"), 
GetSQLValueString($_POST['checkbox5'], "text"),
GetSQLValueString($_POST['checkbox6'], "text"),
GetSQLValueString($_POST['checkbox7'], "text"), 
GetSQLValueString($_POST['checkbox8'], "text"),
GetSQLValueString($_POST['checkbox9'], "text"), 
GetSQLValueString($_POST['checkbox10'], "text"),
GetSQLValueString($_POST['checkbox11'], "text"),
GetSQLValueString($_POST['checkbox12'], "text"));
...
AND (mois = %s OR mois = %s OR mois = %s OR mois = %s OR mois = %s 
	...
	OR mois = %s OR mois = %s) 
GROUP BY personnes.personne_id
ORDER BY personne_nom, annee, mois", 
GetSQLValueString($colname_rsSessions, "int"),
GetSQLValueString($colname_rsSessions2, "text"),
GetSQLValueString($_POST['checkbox13'], "text"),
GetSQLValueString($_POST['checkbox14'], "text"), 
GetSQLValueString($_POST['checkbox15'], "text"),
GetSQLValueString($_POST['checkbox16'], "text"), 
GetSQLValueString($_POST['checkbox17'], "text"),
GetSQLValueString($_POST['checkbox18'], "text"),
GetSQLValueString($_POST['checkbox19'], "text"), 
GetSQLValueString($_POST['checkbox20'], "text"),
GetSQLValueString($_POST['checkbox21'], "text"), 
GetSQLValueString($_POST['checkbox22'], "text"),
GetSQLValueString($_POST['checkbox23'], "text"),
GetSQLValueString($_POST['checkbox24'], "text"));
...
ORDER BY personnes.personne_nom",  GetSQLValueString($colname_rsSessions, "int"),GetSQLValueString($colname_rsSessions2, "text"),GetSQLValueString($_POST['checkbox1'], "text"),GetSQLValueString($_POST['checkbox2'], "text"), GetSQLValueString($_POST['checkbox3'], "text"),GetSQLValueString($_POST['checkbox4'], "text"), GetSQLValueString($_POST['checkbox5'], "text"));
...
AND (mois = %s OR mois = %s OR mois = %s OR mois = %s OR mois = %s 
	...
	OR mois = %s OR mois = %s) 
GROUP BY personnes.personne_nom 
ORDER BY personnes.personne_nom", GetSQLValueString($colname_rsSessions, "int"),
GetSQLValueString($colname_rsSessions2, "text"),
GetSQLValueString($_POST['checkbox1'], "text"),
GetSQLValueString($_POST['checkbox2'], "text"), 
GetSQLValueString($_POST['checkbox3'], "text"),
...
<table width="60%" border="0" align="center">
  ...
  <tr>
    <td colspan="3" align="center">BILAN du mois de <strong>
	<?php $count=0; do  { $count++; ?>
    <?php if (isset($_POST['checkbox'.$count])){ ?>
    <?php echo strtoupper(MinmapDB::getInstance()->get_mois_lib($_POST['checkbox'.$count])).","; ?>
    <?php } ?>
    <?php } while ($count<12)?> <?php echo $_POST['txtYear'] ?></strong></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <?php $counter = 0; $sommer=0; $montant2=0; do  { $counter++; ?>
    <?php if (isset($_POST['checkbox'.$counter])) { ?>
  ...
  <tr>
    ...
    <strong>Sous Commissions Dépendante</strong>
    BILAN du mois de
    <strong>
	<?php $count=13; do  { $count++; ?>
    <?php if (isset($_POST['checkbox'.$count])){ ?>
    <?php echo strtoupper(MinmapDB::getInstance()->get_mois_lib($_POST['checkbox'.$count])).","; ?>
    <?php } ?>
    <?php } while ($count<=25)?> <?php echo $_POST['txtYear'] ?></strong>
    ...
    <table width="50%" border="1" align="center" class="print">
      <tr>
		...
		<?php $counter = 13; do {   ?>
        <?php if (isset($_POST['checkbox'.$counter])) { ?>
        <th nowrap="nowrap"><?php echo $_POST['checkbox'.$counter]; //GetValueMonth($counter);ucfirst($month = MinmapDB::getInstance()->get_month_by_month_id($counter));?>&nbsp;</th>
        <?php } ?>
        <?php $counter++; } while ($counter <= 25) ?>
        <th nowrap="nowrap">Total</th>
        <th>Taux</th>
        <th>Montant</th>
	  ...
	  if (isset($user_rib) && $user_rib <> 'xxxxxxxxxxxxxxxxxxxxxxx'){
        ...
        <?php $counter = 0; $count = 13 ; $sommer=0;  do  {  ?>
        <?php if (isset($_POST['checkbox'.$count])) { ?>
        <td align="right" nowrap="nowrap">
          <?php 

				$nombre_jours = MinmapDB::getInstance()->get_nombre_jour_sCom_by_values($_POST['comID'], $row_rsSessions_sCom['personne_id'], $counter, $_POST['txtYear']);
				if (isset($nombre_jours))
				{ 
					echo $nombre_jours;
					$sommer = $sommer + $nombre_jours;
			...
			?></td>
        <?php } ?>
        <?php $count++; $counter++; } while ($count <= 25) ?>
        ...
        <td align="right" nowrap="nowrap">
		<?php 
				$somme2 = $row_rsSessions_sCom['montant']*$sommer; 
				echo number_format($somme2,0,' ',' ');
			//echo number_format($row_rsSessions_sCom['total'],0,' ',' '); ?>
          <?php  $Montant_global2 = $Montant_global2 + $somme2;?></td>
        <td align="right" nowrap="nowrap">
		<?php echo number_format(($retenu_3 = $somme2*$_SESSION['MM_Retenu']),0,' ',' ');//$row_rsSessions_sCom['nombre_jour']; 
	  $retenu_global_3 = $retenu_global_3 + $retenu_3;
	  ?></td>
        <td align="right" nowrap="nowrap"><strong>
		<?php echo number_format(($net_percu_3 = $somme2-$retenu_3),0,' ',' ');//$row_rsSessions_sCom['nombre_jour']; 
  ...
  <tr>
    ...
    <table border="1" align="center" class="print">
      ...
      <tr>
        ...
        <td align="left" nowrap="nowrap"><a href="#" onclick="<?php popup($showGoToPersonnes3, "700", "400"); ?>"> <?php echo strtoupper($row_rsSessionRepresentant['personne_nom'] . " " . $row_rsSessionRepresentant['personne_prenom']); ?>&nbsp; </a></td>
        ...
        <?php } ?>
        <?php  } while ($count < 12) ?>
        <td align="right" nowrap="nowrap"><?php echo $sommes //$row_rsSessionRepresentant['mois']; ?>&nbsp; </td>
        <td align="right"><?php echo number_format(($row_rsSessionRepresentant['montant']),0,' ',' '); ?>&nbsp; </td>
        <td align="right"><?php $Montant = $sommes*$row_rsSessionRepresentant['montant']; 
					  $Montant_global3 = $Montant_global3 + $Montant; 
					  echo number_format($Montant,0,' ',' '); 
				?>
          &nbsp; </td>
        <td align="right"><?php echo number_format(($retenu_2 = $Montant*$_SESSION['MM_Retenu']),0,' ',' '); ?>
          <?php $retenu_global_2 = $retenu_global_2 + $retenu_2; ?></td>
        <td align="right"><strong><?php echo number_format($net_percu_2 = $Montant-$retenu_2,0,' ',' '); ?></strong>
          <?php $net_percu_global_2 = $net_percu_global_2+$net_percu_2; ?></td>
      </tr>
  ...
  <tr>
    <td colspan="3" align="left">Etat imprimé par : <strong><?php echo strtoupper($_SESSION['MM_Username']); ?></strong> le <?php echo date("d-m-Y"); ?> à <?php echo date("H:i") ?></td>
  </tr>
</table>
</body>
</html>
