...
/*$insertSQL = sprintf("INSERT INTO personnes (personne_id, personne_matricule, personne_nom, personne_prenom, personne_grade, personne_telephone, structure_id, sous_groupe_id, fonction_id, domaine_id, user_id, personne_date_nais, type_personne_id, localite_id, add_commission, add_rep, date_creation, dateUpdate, display) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
MinmapDB::getInstance()->activ_or_desactiv_person_add_rep_by_pers_id($_POST['personne_id'],1, $date);  
...
/*mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsStructure = "SELECT structure_id, structure_lib, code_structure, localite_lib
FROM structures, localites
WHERE structures.localite_id = localites.localite_id AND type_structure_id = 4 AND structures.display = '1'
ORDER BY structure_lib, localite_lib ASC";
$rsStructure = mysql_query($query_rsStructure, $MyFileConnect) or die(mysql_error());
$row_rsStructure = mysql_fetch_assoc($rsStructure);
$totalRows_rsStructure = mysql_num_rows($rsStructure);mysql_select_db($database_MyFileConnect, $MyFileConnect);*/

/*$query_rsStructure = "SELECT structure_id, structure_lib, code_structure, localite_lib FROM structures, localites WHERE structures.localite_id = localites.localite_id AND type_structure_id = 4 AND structures.display = '1' ORDER BY structure_lib, localite_lib ASC";
$rsStructure = mysql_query($query_rsStructure, $MyFileConnect) or die(mysql_error());
$row_rsStructure = mysql_fetch_assoc($rsStructure);
$totalRows_rsStructure = mysql_num_rows($rsStructure);
$query_rsStructure = "SELECT structure_id, structure_lib, code_structure, localite_lib FROM structures, localites WHERE structures.localite_id = localites.localite_id AND type_structure_id = 4 AND structures.display = '1' ORDER BY structure_lib, localite_lib ASC";
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsPersonnes = sprintf("SELECT personne_nom, personne_prenom, personne_telephone FROM personnes WHERE personne_id = %s", GetSQLValueString($colname_rsPersonnes, "int"));
...
<table align="center">
    <tr valign="baseline">
      ...
      <td><input name="personne_nom" type="text" value="<?php echo $row_rsPersonnes['personne_nom']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Prenom :</td>
      <td><input name="personne_prenom" type="text" value="<?php echo $row_rsPersonnes['personne_prenom']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Telephone :</td>
      <td><input name="personne_telephone" type="text" value="<?php echo $row_rsPersonnes['personne_telephone']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
...
do {  
?>
        <option value="<?php echo $row_rsStructure['structure_id']?>"<?php if (!(strcmp($row_rsStructure['structure_id'], $_POST['structure_id']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsStructure['code_structure'].' - '.$row_rsStructure['localite_lib']?></option>
        <?php
...
?>
    ...
    <a href="#" onclick="<?php popup($link, "610", "500"); ?>"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
    </tr>
    <tr valign="baseline">
  ...
  <input type="hidden" name="personne_id" value="<?php echo $_GET['perID'] ?>" />
