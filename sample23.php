...
$nameCommission = (MinmapDB::getInstance()->get_commission_lib_by_commission_id($_GET['comID']));
...
if (isset($_GET['mID'])) {
$link = "sample23.php?comID=".$_GET['comID']."&mID=".$_GET['mID']."&menuID=".$_GET['menuID']."&valid=".$_GET['valid']."&lID=".$row_rsCommissions['localite_id'];
} 
elseif (isset($_POST['annee'])) {
$link = "sample23.php?comID=".$_GET['comID']."&aID=".$_POST['annee']."&menuID=".$_GET['menuID']."&valid=".$_GET['valid']."&lID=".$row_rsCommissions['localite_id'];
} 
elseif (isset($_POST['annee']) && isset($_GET['mID'])) {
$link = "sample23.php?comID=".$_GET['comID']."&aID=".$_POST['annee']."&mID=".$_GET['mID']."&menuID=".$_GET['menuID']."&valid=".$_GET['valid']."&lID=".$row_rsCommissions['localite_id'];
} 
else
{
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsMembres = sprintf("SELECT commission_id, commissions_commission_id, personne_nom, personne_id, personne_prenom, fonction_lib, checboxName, position, fonctions_fonction_id, personnes.structure_id FROM commissions, membres, fonctions, personnes WHERE membres.commissions_commission_id = commissions.commission_id   AND membres.fonctions_fonction_id = fonctions.fonction_id   AND membres.personnes_personne_id = personne_id  AND membres.fonctions_fonction_id BETWEEN 1 AND 3 AND personnes.display = 1  AND commissions_commission_id = %s AND personnes.add_commission = 2 AND membres.display = 1 ORDER BY position, fonctions_fonction_id, personnes.personne_id ", GetSQLValueString($colname_rsMembres, "int"));
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsRepresentant = sprintf("SELECT commission_id, commissions_commission_id, personne_nom, personne_id, personne_prenom, fonction_lib, checboxName, position, fonctions_fonction_id, code_structure, structures.structure_id, structures.localite_id FROM commissions, membres, fonctions, personnes, structures  WHERE membres.commissions_commission_id = commissions.commission_id   AND fonctions_fonction_id = fonctions.fonction_id   AND personnes_personne_id = personne_id  AND personnes.structure_id = structures.structure_id AND membres.fonctions_fonction_id <> 1 AND membres.fonctions_fonction_id <> 2 AND membres.fonctions_fonction_id <> 3 AND personnes.display = 1  AND membres.display = 1 AND commissions_commission_id = %s ORDER BY personnes.dateUpdate, personnes.personne_nom ", GetSQLValueString($colname_rsRepresentant, "int"));
...
$Result = mysql_query($insertSQL, $MyFileConnect) or die(mysql_error());
  ...
  $insertSQL = sprintf("INSERT INTO listing_action (UserId, PageLink, ModificationType, DateCreation) VALUES (%s, %s, %s, %s)",
                       ...
                       GetSQLValueString('A saisie la session du mois de '.$_GET['mID'].'/'.$_POST['annee'].' pour '.$row_rsCommissions['commission_lib'], "text"),
...
?>
    ...
    <tr valign="baseline">
      ...
      <h1>
        ...
        <table width="50%" border="1" class="std">
          <tr>
            <th colspan="3">Jours
            </th>
    ...
    <tr valign="baseline">
      ...
      <table border="1" class="std">
	 	  ...
	 	  ?>
              <img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle" />
              <?php           
			$showGoTo = "search_rep.php?modID=1";
			if (isset($_SERVER['QUERY_STRING'])) {
				$showGoTo .= (strpos($showGoTo, '?')) ? "&" : "?";
				$showGoTo .= $_SERVER['QUERY_STRING'];
			}
	 	  ?>
            <a href="#" onclick="<?php popup($showGoTo, "710", "450"); ?>">Rechercher et ajouter un membre &agrave; la Commission...</a><strong>&nbsp;</strong></td>
          </tr>
          <tr>
            <th>ID</th>
          ...
          <tr>
            ...
            <td>
            <?php           
			$showGoTo = "mod_person.php?perID="
			.$row_rsMembres['personne_id']."&strID="
			.$row_rsMembres['structure_id']."&locID="
			.$_GET['lID']."&action=upd";
			/*if (isset($_SERVER['QUERY_STRING'])) {
				$showGoTo .= (strpos($showGoTo, '?')) ? "&" : "?";
				$showGoTo .= $_SERVER['QUERY_STRING'];
	 	  ...
	 	  ?>
          <a href="#" onclick="<?php popup($showGoTo, "710", "450"); ?>"><?php echo strtoupper($row_rsMembres['personne_nom'] . " " . $row_rsMembres['personne_prenom']); ?><img src="images/img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a>
<input type="hidden" name="<?php echo "personne_id".$compter; ?>" value="<?php echo $row_rsMembres['personne_id']; ?>" size="32" />
            ...
            <td>
            <?php           
			$showGoTo = "mod_membre.php?comID="
			.$row_rsMembres['commission_id']."&fonID="
			.$row_rsMembres['fonctions_fonction_id']."&perID="
			.$row_rsMembres['personne_id']."&locID="
			.$row_rsMembres['localite_id']."&action=upd";
			/*if (isset($_SERVER['QUERY_STRING'])) {
				$showGoTo .= (strpos($showGoTo, '?')) ? "&" : "?";
				$showGoTo .= $_SERVER['QUERY_STRING'];
			}*/
	 	  ?><a href="#" onclick="<?php popup($showGoTo, "710", "450"); ?>"><?php echo $row_rsMembres['fonction_lib']; ?><img src="images/img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a>&nbsp;
		  ...
		  <?php echo ((in_array($value, $liste))? "checked='checked'":"") ?> /></td>
            <?php break; } } ?>
            <?php //} while($counter < $row_rsJourMois['nbre_jour']); ?>
            <td><a href="activ_rep.php?comID=<?php echo $_GET['comID']; ?>&lID=<?php echo $_GET['lID']; ?>&amp;perID=<?php echo $row_rsMembres['personne_id'];?>&amp;mID=<?php echo $_GET['mID']; ?>&amp;action=desactive&amp;menuID=<?php echo $_GET['menuID']; ?>&page=sample23"><img src="images/img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
          </tr>
    ...
    <tr valign="baseline">
      ...
      <h1>
        <?php if (isset($_GET['comID']) && isset($_GET['mID'])) { // Show if recordset not empty ?>
        Representant maitre d'ouvrage</h1>
      <table border="1" class="std">
          <tr>
            <td colspan="<?php echo ($row_rsJourMois['nbre_jour']+4) ?>"><?php           
			$showGoTo = "sample14.php";
	 	  ...
	 	  ?>
              <img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle" />
              <?php           
			$showGoTo = "search_reps.php?modID=2";
			if (isset($_SERVER['QUERY_STRING'])) {
				$showGoTo .= (strpos($showGoTo, '?')) ? "&" : "?";
				$showGoTo .= $_SERVER['QUERY_STRING'];
			}
	 	  ?>
            <a href="#" onclick="<?php popup($showGoTo, "710", "450"); ?>">Rechercher et ajouter un repr&eacute;sentant Maitre d'Ouvrage...</a><strong>&nbsp;</strong></td>
          </tr>
          <tr>
          ...
          <?php if ($totalRows_rsRepresentant > 0) { // Show if recordset not empty ?>
            ...
            <?php           
			$showGoTo = "mod_person.php?perID="
			.$row_rsRepresentant['personne_id']."&strID="
			.$row_rsRepresentant['structure_id']."&locID="
			.$_GET['lID']."&action=upd";
			/*if (isset($_SERVER['QUERY_STRING'])) {
				$showGoTo .= (strpos($showGoTo, '?')) ? "&" : "?";
				$showGoTo .= $_SERVER['QUERY_STRING'];
	 	  ...
	 	  ?>
          <a href="#" onclick="<?php popup($showGoTo, "710", "450"); ?>"><?php echo strtoupper($row_rsRepresentant['personne_nom'] . " " . $row_rsRepresentant['personne_prenom']); ?><img src="images/img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a>
<input type="hidden" name="<?php echo "representant_id".$count; ?>" value="<?php echo $row_rsRepresentant['personne_id']; ?>" />
              &nbsp; </td>
              <td><?php echo strtoupper($row_rsRepresentant['code_structure']); ?>
          ...
          <?php //} while($counter < $row_rsJourMois['nbre_jour']); ?>
          <td><a href="activ_rep.php?comID=<?php echo $_GET['comID']; ?>&amp;perID=<?php echo $row_rsRepresentant['personne_id'];?>&amp;mID=<?php echo $_GET['mID']; ?>&lID=<?php echo $_GET['lID']; ?>&amp;action=desactive&amp;menuID=<?php echo $_GET['menuID']; ?>&page=sample23"><img src="images/img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
            </tr>
