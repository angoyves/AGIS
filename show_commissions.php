...
$query_rsCommissions = sprintf("SELECT commission_id, commission_lib, lib_nature, type_commission_lib, localite_lib, membre_insert, commissions.region_id FROM commissions, natures, type_commissions, localites WHERE commissions.nature_id = natures.nature_id  AND commissions.type_commission_id = type_commissions.type_commission_id  AND commissions.localite_id = localites.localite_id 
							   AND (commissions.type_commission_id BETWEEN 1  AND 4)
							   AND commissions.type_commission_id = %s AND commissions.commission_id = %s ORDER BY commissions.dateCreation, commissions.type_commission_id, commissions.commission_lib ASC", GetSQLValueString($_GET['typID'], "int"), GetSQLValueString($_GET['comID'], "int"));
...
<?php } ?>
<?php if (isset($_SESSION['MM_UserGroup']) && $_SESSION['MM_UserGroup'] == '1'){ ?>
      <td nowrap="nowrap"><a href="#" onclick="<?php popup('maj_commission.php?comID='.$row_rsCommissions['commission_id'], "700", "400"); ?>"><img src="images/img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/><?php echo MinmapDB::getInstance()->get_region_lib_by_region_id($row_rsCommissions['region_id']) ?></a></td>
<?php } ?>
      </tr>
