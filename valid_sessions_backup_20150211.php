...
if (empty($_POST['nombre_dossier'])){
    	$dossierIsEmpty = true;
    }
if (empty($_GET['moisID'])){
    	$moisIsEmpty = true;
    }
...
if ((isset($_POST['personne_id1']) && empty($variable1)) || (isset($_POST['personne_id2']) && empty($variable2)) || (isset($_POST['personne_id3']) && empty($variable3)) || (isset($_POST['personne_id4']) && empty($variable4)) || (isset($_POST['personne_id5']) && empty($variable5)) || (isset($_POST['personne_id7']) && empty($variable7)) || (isset($_POST['personne_id8']) && empty($variable8)) || (isset($_POST['personne_id9']) && empty($variable9)) || (isset($_POST['personne_id6']) && empty($variable6)))
{
	$JourIsEmpty = true;
	}
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1") && !$anneeIsEmpty && !$dossierIsEmpty && !$moisIsEmpty && $sessionIsUnique && !$JourIsEmpty) {
  ...
  <table width="75%" border="0" align="center">
    <tr>
      <td><table width="50%" align="left" class="std">
        <tr valign="baseline">
          <th nowrap="nowrap" align="right">Mois <span class="error">*</span> :</th>
          <td><select name="jumpMenu" id="jumpMenu" onchange="MM_jumpMenu('parent',this,0)">
            <option value="">:: Select...</option>
            <?php
do {  
?>
            <option value="valid_sessions.php?menuID=<?php echo $_GET['menuID'] ?>&comID=<?php echo $_GET['comID'] ?>&amp;moisID=<?php echo $row_rsMois['mois_id']?>" <?php if (!(strcmp($_GET['moisID'], $row_rsMois['mois_id']))) {echo "SELECTED";} ?>><?php echo ucfirst($row_rsMois['lib_mois']) ?></option>
            <?php
...
?>
    ...
    <tr>
      <td><table width="75%" border="1" class="std">
        ...
        <tr valign="baseline">
          <th nowrap="nowrap" align="right">Annee <span class="error">*</span> :</th>
          <td nowrap="nowrap"><select name="annee">
            <option value="" <?php if (!(strcmp("", $_POST['annee']))) {echo "selected=\"selected\"";} ?>>...</option>
            <?php
do {  
?>
<option value="<?php echo $row_rsAnnee['lib_annee']?>"<?php if (!(strcmp($row_rsAnnee['lib_annee'], $_POST['annee']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsAnnee['lib_annee']?></option>
            <?php
...
?>
       ...
       if ($anneeIsEmpty) { ?>
            ...
            <?php  } ?></td>
          <th align="right" nowrap="nowrap">Nombre de dossiers <span class="error">*</span> :</th>
          <td nowrap="nowrap"><input type="text" name="nombre_dossier" value="<?php echo $_POST['nombre_dossier']; ?>" size="15" />
            <?php
             /** Display error messages if the "password" field is empty */
            if ($dossierIsEmpty) { ?>
        ...
        <tr valign="baseline">
          ...
          <td colspan="3" valign="top"><textarea name="dossier_id" id="dossier_id" cols="45" rows="5"><?php echo $_POST['dossier_id']; ?></textarea>
<?php           
...
?>
    ...
    <tr>
      ...
      <?php if (!$sessionIsUnique) {
    	$link2 = "upd_sessions.php?aID=" . $_POST['annee']. "&mID=" . $_GET['moisID'] . "&comID=" . $_GET['comID'] . "&menuID=" . $_GET['menuID'] ?>
    	<strong><span class="error"><span class="control"><img src="images/img/s_attention.png" alt="" width="16" height="16" align="absmiddle" /></span> Cette Session existe déjà dans le système...Vous souhaitez le: </span></strong><a href="<?php echo $link2 ?>"><img src="images/img/b_edit.png" width="16" height="16" /> Modifier</a>
	    <?php } ?>
        <h1>Membres</h1>

      <?php if ($JourIsEmpty) { ?>
      <span class="control"><img src="images/img/s_attention.png" alt="" width="16" height="16" align="absmiddle" /></span>    	<strong><span class="error">Saisie des sessions de tous les membres obligatoires... </span></strong>
	  <?php } ?>
	...
	?>
          ...
          <tr>
            ...
            <?php 
	$counter=0; do  { $counter++; ?>
            <td nowrap="nowrap"><input  name="<?php echo "jour".$compter.$counter."[]"; ?>" type="checkbox" id="<?php echo "jour".$compter.$counter; ?>" value="<?php echo $counter ?>" <?php if (!(strcmp($_POST['jour'.$compter.$counter],"$counter"))) {echo "checked=\"checked\"";} ?>/></td>
            <?php }while ($counter< $row_rsJourMois['nbre_jour']);
	?>
          </tr>
        ...
        <input type="hidden" name="NombreJour" value="<?php echo $row_rsJourMois['nbre_jour']; ?>" size="32" /></td>
    </tr>
  </table>
</form>
<p>&nbsp;</p>
