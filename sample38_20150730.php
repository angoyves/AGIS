...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsSousCommission = sprintf("SELECT commission_id, personnes.personne_id, personne_nom, fonction_lib, nombre_jour, jour, mois, annee, montant, montant_cumul, nombre_offre, (nombre_jour * montant) as total FROM commissions, membres, personnes,  fonctions, sessions WHERE membres.commissions_commission_id = commissions.commission_id AND membres.personnes_personne_id = personnes.personne_id AND membres.fonctions_fonction_id = fonctions.fonction_id AND sessions.membres_commissions_commission_id = membres.commissions_commission_id AND sessions.membres_personnes_personne_id = membres.personnes_personne_id AND (membres.fonctions_fonction_id BETWEEN 1 AND 3 OR membres.fonctions_fonction_id = 41) AND commission_parent = %s AND mois = %s AND annee = %s ORDER BY fonctions.fonction_id", GetSQLValueString($colname_rsSousCommission, "int"),GetSQLValueString($colname1_rsSousCommission, "int"),GetSQLValueString($colname2_rsSousCommission, "int"));
...
<table>
  <tr>
    <td><a href="#" onclick="<?php popup($link13, "1200", "700") ?>"><img src="images/img/b_print.png" width="16" height="16" /> Version imprimable</a>&nbsp; <a href="#" onclick="<?php popup($link, "1200", "700") ?>"><img src="images/img/b_print.png" width="16" height="16" /> Imprimer Etat</a>&nbsp; <a href="#" onclick="<?php popup($link11, "1200", "700") ?>"><img src="images/img/b_print.png" width="16" height="16" /> Imprimer Recapitulatif </a>&nbsp;&nbsp;
...
<h1>Sous Commissions</h1>
  <table border="1" align="center" class="std">
    ...
    <tr>
      ...
      <td><?php echo MinmapDB::getInstance()->get_dossier_ref_by_value($row_rsSousCommission['commission_id'], $_POST['txt_Month'], $_POST['txt_Year']); ?>&nbsp;</td>
      <td valign="top"><?php
	  	$colname_rsMembres = "-1";
	...
	?>
        <table width="100%" align="left" id="*">
          ...
          <?php do { ?>
            <tr>
              <td><a href="detail_membres_scom.php?recordID=<?php echo $row_rsMembres['commissions_commission_id']; ?>"><?php echo strtoupper(MinmapDB::getInstance()->get_personne_name_by_person_id($row_rsMembres['personnes_personne_id'])); ?></a></td>
              <td><?php echo strtoupper(MinmapDB::getInstance()->get_fonction_lib_by_fonction_id($row_rsMembres['fonctions_fonction_id'])); ?>&nbsp; </td>
              <td align="right"><?php echo number_format($row_rsMembres['montant'],0,' ',' '); ?>&nbsp; </td>
            </tr>
            <?php } while ($row_rsMembres = mysql_fetch_assoc($rsMembres)); ?>
      </table></td>
      <td align="center"><strong><?php echo number_format($row_rsSousCommission['montant_cumul'],0,' ',' '); ?></strong></td>
