...
// FONCTION DE CONTROLE DES MOIS DANS LA TABLE APPUREMENT
    ...
    public function get_montant_appurement_by_rib($rib) {
		$rib = $this->real_escape_string($rib);
        $domain = $this->query("SELECT montant
									FROM appurements
...
// COMMISSIONS
	 ...
	 function get_sum_indemnite_by_commission_id($commission_id, $type_commission_id) {
		//$champ = $this->real_escape_string($champ);
        $domain = $this->query("SELECT sum(Nombre_jour * montant) FROM sessions, membres, commissions
								...
								AND (type_commission_id = ".$type_commission_id." OR type_commission_id = 6)
