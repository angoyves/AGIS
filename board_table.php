...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsUsers = "SELECT user_id, user_name, user_lastname, users.structure_id, user_groupe_id, structure_lib, compteur, date_last_login, users.display, user_connected FROM users, structures WHERE users.structure_id = structures.structure_id";
...
<!--div class="bloc express">
    ...
    <div class="bloc lois_recentes">
          <ul class="tous_les">
            <strong>Notifications</strong> Vous avez  :<strong> 
			<?php echo MinmapDB::getInstance()->get_nbre_commentaire_non_lu($_SESSION['MM_UserID']); ?></strong>
          <?php           
			$showGoTo = "sample50.php?userID=".$_SESSION['MM_UserID'];
			if (isset($_SERVER['QUERY_STRING'])) {
	 	  ...
	 	  ?>
          <a href="#" onclick="<?php popup($showGoTo, "710", "350"); ?>"><img src="images/img/message.png" width="28" height="18" border="0" align="middle">Nouveaux Messages</a>
          de 
          </ul>
    </div>
    <h1>Tableau de Bord - Etat de saisie dans le système 
	<?php 
/*  $userConnected = MinmapDB::getInstance()->get_user_connexion($_SESSION['MM_UserID']);
  if (isset($userConnected) || ($userConnected == 0)){
	  session_unset();
	  }*/
	?>
<?php //echo $userConnected = MinmapDB::getInstance()->get_user_connexion($_SESSION['MM_UserID']); ?></h1><table border="1" align="center" class="std">
      <tr align="center">
        <th>ID</th>
        <th>Nom et prenom</th>
        <th>Structures</th>
        ...
        <?php if(isset($UserGroupe) && $UserGroupe==1){ ?>
        <th>Groupe</th>
        <th>Activ&eacute;</th>
        <th>Connect&eacute;</th>
        <th>Mot de passe</th>
        <?php } ?>
      </tr>
	  ...
	  <tr bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
          ...
          <td>
          <?php if(isset($UserGroupe) && $UserGroupe==$row_rsUsers['user_groupe_id']){ 
		  $link = 'sample56.php?uID='.$row_rsUsers['user_id'];
		  ?>
          <a href="#" onclick="<?php popup($link, "700", "400"); ?>">
		  <?php echo strtoupper($row_rsUsers['user_name']). " " . ucfirst($row_rsUsers['user_lastname']); ?>
          </a>
          <?php
		  } else { 
		  echo strtoupper($row_rsUsers['user_name']). " " . ucfirst($row_rsUsers['user_lastname']);
		  }
		  ?>&nbsp;
          ...
          <td align="center"><?php echo $row_rsUsers['date_last_login']; // = (MinmapDB::getInstance()->get_user_groupe_by_user_login($_SESSION['MM_Username']));//echo $_SESSION['MM_UserGroup']; $UserGroupe =(MinmapDB::getInstance()->get_user_group_id_by_user_id($row_rsUsers['user_id'])); ?>&nbsp; </td>
          <?php if(isset($UserGroupe) && $UserGroupe==1){  
		  $link = 'sample55.php?userID='.$row_rsUsers['user_id'];
		  ?>
        <td align="left">
          <a href="#" onclick="<?php popup($link, "700", "400"); ?>">
	    <?php 
		  	$group_id = (MinmapDB::getInstance()->get_user_group_id_by_user_id($row_rsUsers['user_id'])); 
		  	echo MinmapDB::getInstance()->get_user_groupe_name_by_group_id($group_id)

		  ?></a></td>
        <td align="center">          
		<?php if (isset($row_rsUsers['display']) && $row_rsUsers['display'] == '1') { ?>
        ...
        <td align="center"><?php if (isset($row_rsUsers['user_connected']) && $row_rsUsers['user_connected'] == '1') { ?>
        <a href="sample97.php?recordID=<?php echo $row_rsUsers['user_id']; ?>&map=users&mapid=user_id&action=desactive&menuID=<?php echo $_GET['menuID']; ?>&page=index.php&col=user_connected">
        <img src="images/img/s_okay.png" alt="" width="16" height="16" align="absmiddle"/>
        </a>
        <?php } else { ?>
        <img src="images/img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/>
        <?php } ?></td>
        <td align="center">
        <a href="sample97.php?recordID=<?php echo $row_rsUsers['user_id']; ?>&map=users&mapid=user_id&action=init&menuID=<?php echo $_GET['menuID']; ?>&page=index.php&col=user_password" onClick="return confirm('Etes vous sur de vouloir reinitialiser le mot de passe ?');">
