...
$maxRows_rsMbre_Par_Commission = 10;
$pageNum_rsMbre_Par_Commission = 0;
if (isset($_GET['pageNum_rsMbre_Par_Commission'])) {
  $pageNum_rsMbre_Par_Commission = $_GET['pageNum_rsMbre_Par_Commission'];
}
$startRow_rsMbre_Par_Commission = $pageNum_rsMbre_Par_Commission * $maxRows_rsMbre_Par_Commission;

mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsMbre_Par_Commission = "SELECT commission_id, count(commissions_commission_id) as Nombre, localite_lib, lib_nature, commission_lib  FROM commissions, membres, natures, localites WHERE commissions.commission_id = membres.commissions_commission_id AND commissions.nature_id = natures.nature_id AND commissions.localite_id = localites.localite_id";
$query_limit_rsMbre_Par_Commission = sprintf("%s LIMIT %d, %d", $query_rsMbre_Par_Commission, $startRow_rsMbre_Par_Commission, $maxRows_rsMbre_Par_Commission);
$rsMbre_Par_Commission = mysql_query($query_limit_rsMbre_Par_Commission, $MyFileConnect) or die(mysql_error());
$row_rsMbre_Par_Commission = mysql_fetch_assoc($rsMbre_Par_Commission);

if (isset($_GET['totalRows_rsMbre_Par_Commission'])) {
  $totalRows_rsMbre_Par_Commission = $_GET['totalRows_rsMbre_Par_Commission'];
} else {
  $all_rsMbre_Par_Commission = mysql_query($query_rsMbre_Par_Commission);
  $totalRows_rsMbre_Par_Commission = mysql_num_rows($all_rsMbre_Par_Commission);
}
$totalPages_rsMbre_Par_Commission = ceil($totalRows_rsMbre_Par_Commission/$maxRows_rsMbre_Par_Commission)-1;
...
$queryString_rsMbre_Par_Commission = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rsMbre_Par_Commission") == false && 
        stristr($param, "totalRows_rsMbre_Par_Commission") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rsMbre_Par_Commission = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rsMbre_Par_Commission = sprintf("&totalRows_rsMbre_Par_Commission=%d%s", $totalRows_rsMbre_Par_Commission, $queryString_rsMbre_Par_Commission);
?>
...
<table border="1" align="center" class="std">
  <tr>
    ...
    <th class="std">Etat</th>
    <th class="std">Edit</th>
    <th class="std">Del</th>
    <th class="std">Detail</th>
  </tr>
  <?php do { ?>
    <tr>
      <td nowrap="nowrap"><a href="details_sessions.php?recordID=<?php echo $row_rsMbre_Par_Commission['commission_id']; ?>"> Details&nbsp; </a></td>
      <td nowrap="nowrap"><?php echo $row_rsMbre_Par_Commission['commission_lib']; ?>&nbsp; </td>
      <td nowrap="nowrap"><?php echo $row_rsMbre_Par_Commission['localite_lib']; ?>&nbsp; </td>
      <td nowrap="nowrap"><?php echo $row_rsMbre_Par_Commission['Nombre']; ?>&nbsp; </td>
      <td nowrap="nowrap" bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>" class="std"><?php if (isset($row_rsMbre_Par_Commission['display']) && $row_rsMbre_Par_Commission['display'] == '1') { ?>
        <a href="change_etat.php?recordID=<?php echo $row_rsMbre_Par_Commission['commission_id']; ?>&amp;&amp;map=commissions&amp;&amp;mapid=commission_id&amp;&amp;action=desactive"><img src="images/img/s_okay.png" alt="" width="16" height="16" align="absmiddle"/></a>
        <?php } else { ?>
        <a href="change_etat.php?recordID=<?php echo $row_rsMbre_Par_Commission['commission_id']; ?>&amp;&amp;map=commissions&amp;&amp;mapid=commission_id&amp;&amp;action=active"><img src="images/img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a>
        <?php } ?></td>
      <td nowrap="nowrap" bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>" class="std"><a href="edit_commissions.php?recordID=<?php echo $row_rsMbre_Par_Commission['commission_id']; ?>&amp;&amp;map=commissions"><img src="images/img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
      <td nowrap="nowrap" bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>" class="std"><a href="delete.php?recordID=<?php echo $row_rsMbre_Par_Commission['commission_id']; ?>&amp;&amp;map=commissions"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
      <td nowrap="nowrap" bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>" class="std"><a href="afficher_membres.php?recordID=<?php echo $row_rsMbre_Par_Commission['commission_id']; ?>"><img src="images/img/b_view.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
    </tr>
    <?php } while ($row_rsMbre_Par_Commission = mysql_fetch_assoc($rsMbre_Par_Commission)); ?>
</table>
<br />
<table border="0">
  <tr>
    <td><?php if ($pageNum_rsMbre_Par_Commission > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_rsMbre_Par_Commission=%d%s", $currentPage, 0, $queryString_rsMbre_Par_Commission); ?>">First</a>
    <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_rsMbre_Par_Commission > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_rsMbre_Par_Commission=%d%s", $currentPage, max(0, $pageNum_rsMbre_Par_Commission - 1), $queryString_rsMbre_Par_Commission); ?>">Previous</a>
    <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_rsMbre_Par_Commission < $totalPages_rsMbre_Par_Commission) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_rsMbre_Par_Commission=%d%s", $currentPage, min($totalPages_rsMbre_Par_Commission, $pageNum_rsMbre_Par_Commission + 1), $queryString_rsMbre_Par_Commission); ?>">Next</a>
    <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_rsMbre_Par_Commission < $totalPages_rsMbre_Par_Commission) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_rsMbre_Par_Commission=%d%s", $currentPage, $totalPages_rsMbre_Par_Commission, $queryString_rsMbre_Par_Commission); ?>">Last</a>
    <?php } // Show if not last page ?></td>
  </tr>
</table>
Records <?php echo ($startRow_rsMbre_Par_Commission + 1) ?> to <?php echo min($startRow_rsMbre_Par_Commission + $maxRows_rsMbre_Par_Commission, $totalRows_rsMbre_Par_Commission) ?> of <?php echo $totalRows_rsMbre_Par_Commission ?>
<!-- InstanceEndEditable -->
    <!--div class="bloc express">
				<h4>Informations express</h4>
				<p><span>9 mars 2009</span><br>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsMbre_Par_Commission);
