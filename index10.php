...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsRegions = "SELECT * FROM regions WHERE display = '1' ORDER BY region_align ASC";
...
<table border="1" align="center" class="std">
  <tr>
    ...
    <th nowrap>MONTANT PAYE</th>
    <th>OBSERVATION</th>
  </tr>
  <?php 
  $somme_total=0; 
  $net_a_percevoir_total=0;
  $count=0; 
  do { $count++; ?>
    <tr>
      <td height="23" colspan="6" align="center"><strong>REGION : <?php echo $row_rsRegions['region_lib']; ?></strong>&nbsp;   </td>
    </tr>
    <?php 
    mysql_select_db($database_MyFileConnect, $MyFileConnect);
	...
	?>
    <?php $counter=0; ; do  { $counter++; $count++; ?>
    <tr bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
      ...
      <td><?php echo strtoupper($row_rsCommissionsR['commission_lib']); ?></td>
      <td align="right"><?php echo number_format($montant_brut = MinmapDB::getInstance()->get_sum_indemnite_by_commission_id($row_rsCommissionsR['commission_id'], $txtYear, $txtMonth1, $txtMonth2),0,' ',' '); 
	  $somme_brut=0; 
	  $somme_retenu=0;
	  $net_a_percevoir=0;
	  $retenu = MinmapDB::getInstance()->get_retenu($txtYear);
	  $somme_brut = $somme_brut + $montant_brut;
	  $somme_retenu = $somme_brut * $retenu;
	  $net_a_percevoir = $somme_brut - $somme_retenu;
	  
	  ?></td>
      <td align="right"><?php echo number_format($net_a_percevoir,0,' ',' '); //$row_rsCommissionsR['nombre_offre']; ?></td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
  </tr>
    ...
    <tr bgcolor="#CACACA">
      ...
      <td align="right"><strong><?php echo number_format($somme_brut,0,' ',' '); 
	  
	  $somme_total = $somme_total + $somme_brut; 

	  ?></strong></td>
      <td align="right"><strong><?php echo number_format($net_a_percevoir,0,' ',' ');
	  
	  	  $net_a_percevoir_total = $net_a_percevoir_total + $net_a_percevoir; ?></strong></td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
    <?php } while ($row_rsRegions = mysql_fetch_assoc($rsRegions)); ?>
	<tr>
      <td height="32" colspan="2"><strong>TOTAL GENERAL</strong></td>
      <td align="right"><strong><?php echo number_format($somme_total,0,' ',' '); ?></strong></td>
      <td align="right"><strong><?php echo number_format($net_a_percevoir_total,0,' ',' '); ?></strong></td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
</table>
