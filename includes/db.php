...
//OV
    ...
    public function get_montant_ov($txtSearch) {
		$txtSearch = $this->real_escape_string($txtSearch);
        $domain = $this->query("SELECT sum(montant) 
		FROM appurements WHERE (motif LIKE '%"
		. $txtSearch . "%' OR nom_beneficiaire LIKE '%"
		. $txtSearch . "%' OR rib_beneficiaire LIKE '%" 
		. $txtSearch . "%' OR ref_dossier LIKE '%"
		. $txtSearch . "%') AND display = 1 ORDER BY nom_beneficiaire ASC");
...
// PARAMETRES
    ...
    public function get_taux($year) {
		
        $domain = $this->query("SELECT Taux
									FROM params
									WHERE exercice = ". $year);
...
// TYPE LOCALITE 
	public function change_etat_index($table, $champ_id, $value_id, $display) {
		$table = $this->real_escape_string($table);
		$champ_id = $this->real_escape_string($champ_id);
        $domain = $this->query("UPDATE appurements SET display = ". $display ." WHERE appurement_id = ". $value_id );
	...
	public function get_count_by_id($table, $champ_id, $value_id) {
		$table = $this->real_escape_string($table);
		$champ_id = $this->real_escape_string($champ_id);
        $domain = $this->query("SELECT count(". $champ_id .") FROM ". $table ." WHERE ". $champ_id ." = ". $value_id );
...
// PERSONNES
	...
	public function update_row($table, $colNameOne, $colNameTwo, $colWhere, $enterValueOne, $enterValueTwo, $enterValueWhere) {
        $this->query("UPDATE ". $table ." SET ". $colNameOne ." = ". $enterValueOne . " WHERE ". $colNameTwo ." = ".  $enterValueTwo
					 . " AND ". $colWhere ." = " . $enterValueWhere);
    }
...
// MEMBRES
	...
	public function activ_or_desactiv_membre($Personne_id, $display, $duedate, $commission_id, $fonction_id) {
		$display = $this->real_escape_string($display);
        $this->query("UPDATE membres SET display='" . $display . "', dateUpdate = " . $this->format_date_for_sql($duedate)
					 . " WHERE commissions_commission_id = " . $commission_id 
					 . " AND fonctions_fonction_id= " . $fonction_id 
...
*/
    public function get_montant_appurement_by_rib($rib, $txtSearch, $commission_id, $annee) {
		$rib = $this->real_escape_string($rib);
		$txtSearch = $this->real_escape_string($txtSearch);
		$annee = $this->real_escape_string($annee);
        $domain = $this->query("SELECT sum(montant)
									FROM appurements
									WHERE rib_beneficiaire = '". $rib 
									. "' AND nom_beneficiaire LIKE '%" . $txtSearch
									. "%' AND commission_id = " . $commission_id 
									. " AND annee = '" . $annee 
									. "' AND display = 1" );
...
//SESSIONS
    public function update_session_person_day($nbre_jours, $jour, $duedate, $mois_new, $mois_old, $annee_new, $annee_old, $commission_id, $personne_id) { 
		$nbre_jours = $this->real_escape_string($nbre_jours);
		$jour = $this->real_escape_string($jour); 
		$mois_new = $this->real_escape_string($mois_new); 
		$mois_old = $this->real_escape_string($mois_old);
		$annee_new = $this->real_escape_string($annee_new); 
		$annee_old = $this->real_escape_string($annee_old);
	...
	public function validate_sessions($value, $duedate, $mois, $annee, $commission_id, $UserId, $champ) { 
		$value = $this->real_escape_string($value);
		$champ = $this->real_escape_string($champ);
		$mois = $this->real_escape_string($mois); 
		$annee = $this->real_escape_string($annee); 
        $this->query("UPDATE sessions SET etat_validation = '" . $value
					 . "', dateUpdate = " . $this->format_date_for_sql($duedate)
					 . ", ". $champ . " = " . $UserId 
					 . " WHERE membres_commissions_commission_id = " . $commission_id 
					 . " AND mois = '" . $mois 
	...
	public function validate_sessions_all($value, $duedate, $annee, $commission_id, $UserId, $champ) { 
		$value = $this->real_escape_string($value);
		$champ = $this->real_escape_string($champ); 
		$annee = $this->real_escape_string($annee); 
        $this->query("UPDATE sessions SET etat_validation = '" . $value
...
// FIN COMMISSIONS
	...
	public function get_dossier_id_by_value($commission_id, $mois, $annee) {
		$mois = $this->real_escape_string($mois);
		$annee = $this->real_escape_string($annee);
        $domain = $this->query("SELECT dossiers_dossier_id FROM sessions WHERE membres_commissions_commission_id =" . 	$commission_id . " AND mois ='" . $mois . "' AND annee = '" . $annee . "'" );
    ...
    public function get_nombre_jour_by_values($commission_id, $personne_id, $month, $year) {
        ...
        $domain = $this->query("SELECT nombre_jour FROM sessions WHERE sessions.membres_personnes_personne_id = " . $personne_id 
							   . " AND sessions.membres_commissions_commission_id = " . $commission_id 
    ...
    public function get_nombre_jour_by_year($commission_id, $personne_id, $month1, $month2, $year) {
		$year = $this->real_escape_string($year);
		//$month = $this->real_escape_string($month);
        $domain = $this->query("SELECT sum(nombre_jour) FROM sessions 
								WHERE sessions.membres_personnes_personne_id = " . $personne_id 
							   . " AND sessions.membres_commissions_commission_id = " . $commission_id 
							   . " AND sessions.mois BETWEEN " . $month1 . " AND " . $month2
							   . " AND sessions.annee = '" . $year 
							   . "' AND etat_validation = 2 AND etat_appur = 0 ");
	    ...
	    public function get_nombre_jour_sCom_by_values2($commission_id, $personne_id, $fonction, $month, $year) {
		$year = $this->real_escape_string($year);
		//$month = $this->real_escape_string($month);
        $domain = $this->query("SELECT count(sessions.Nombre_jour) FROM sessions, commissions 
								WHERE sessions.membres_commissions_commission_id = commissions.commission_id
	    ...
	    public function get_nombre_jour_sCom_by_year($commission_id, $personne_id, $fonction, $txtMonth1, $txtMonth2, $year) {
		$year = $this->real_escape_string($year);
		//$month = $this->real_escape_string($month);
        $domain = $this->query("SELECT sum(sessions.Nombre_jour) FROM sessions, commissions 
								WHERE sessions.membres_commissions_commission_id = commissions.commission_id
								AND commissions.commission_parent = " . $commission_id 
								. " AND sessions.membres_personnes_personne_id = " . $personne_id 
								. " AND sessions.mois BETWEEN " . $txtMonth1 . " AND " . $txtMonth2
								. " AND sessions.annee = '" . $year
								. "' AND etat_validation = 2 AND etat_appur = 0 ");
	    ...
	    public function get_nombre_jour_com_by_year($commission_id, $personne_id, $fonction, $txtMonth1, $txtMonth2, $year) {
		$year = $this->real_escape_string($year);
		//$month = $this->real_escape_string($month);
        $domain = $this->query("SELECT sum(sessions.Nombre_jour) FROM sessions, commissions 
								WHERE sessions.membres_commissions_commission_id = commissions.commission_id
								AND commissions.commission_id = " . $commission_id 
								. " AND sessions.membres_personnes_personne_id = " . $personne_id 
								. " AND sessions.mois BETWEEN " . $txtMonth1 . " AND " . $txtMonth2
								. " AND sessions.annee = '" . $year
								. "' AND etat_validation = 2 AND etat_appur = 0 ");
	    ...
	    public function get_nbr_day_sCom_by_values($commission_id, $personne_id, $fonction, $month1, $month2, $year) {
		$year = $this->real_escape_string($year);
		//$month = $this->real_escape_string($month);
        $domain = $this->query("SELECT count(sessions.Nombre_jour) FROM sessions, commissions 
								WHERE sessions.membres_commissions_commission_id = commissions.commission_id
								AND commissions.commission_parent = " . $commission_id 
								. " AND sessions.membres_personnes_personne_id = " . $personne_id 
								. " AND sessions.membres_fonctions_fonction_id = " . $fonction
								. " AND (sessions.mois BETWEEN " . $month1 
								. " AND " . $month2 . ")"
								. " AND sessions.annee = '" . $year
								. "' AND etat_validation = 2 AND etat_appur = 0 ");
	    ...
	    public function SelectMontantByFonctionTYpe($commission_id, $personne_id, $fonction, $txtMonth1, $txtMonth2, $year) {
		$year = $this->real_escape_string($year);
		//$month = $this->real_escape_string($month);
        $domain = $this->query("SELECT (sum(nombre_jour) * montant) as total
		FROM sessions, membres, commissions, personnes
		WHERE sessions.membres_commissions_commission_id = membres.commissions_commission_id 
		AND sessions.membres_personnes_personne_id = membres.personnes_personne_id
		AND commissions.commission_id = membres.commissions_commission_id
    ...
    public function get_montant_person_by_values($commission_id, $personne_id, $fonction_id) {
        $domain = $this->query("SELECT montant FROM membres WHERE commissions_commission_id = ". $commission_id 
				. " AND personnes_personne_id = " . $personne_id 
				. " AND fonctions_fonction_id = " . $fonction_id);
        if ($domain->num_rows > 0){
            $row = $domain->fetch_row();
	    ...
	    public function get_person_montant_day_sCom_by_values($commission_id, $personne_id, $fonction, $month1, $month2, $year) {
		$year = $this->real_escape_string($year);
		//$month = $this->real_escape_string($month);
        $domain = $this->query("SELECT count(sessions.Nombre_jour) FROM sessions, commissions 
								WHERE sessions.membres_commissions_commission_id = commissions.commission_id
	...
	// fonction qui donne l'etat de la commission ou de la sous commission
	function afficher_etat_valide2($comID, $mID, $aID, $scomID) {
	    ...
	    $domain = $this->query("SELECT etat_validation FROM sessions WHERE mois= '" . $mID 
							   ...
							   . " OR membres_commissions_commission_id = " . $scomID .")");
	...
	function afficher_user_sessions($comID, $mID, $aID, $champ) {
        $mID = $this->real_escape_string($mID);
		$aID = $this->real_escape_string($aID);
		$champ = $this->real_escape_string($champ);
	    $domain = $this->query("SELECT " . $champ . " FROM sessions WHERE mois= '" . $mID 
							   . "' AND annee= '" . $aID 
							   . "' AND membres_commissions_commission_id = " . $comID);
  ...
  /*  public function create_listing_action($UserId, $PageLink, $ModificationType, $DateCreation) {
    ...
    public function verify_membre($commission_id, $personne_id, $fonction_id) {
        $result = $this->query("SELECT 1 FROM membres WHERE commissions_commission_id = ". $commission_id 
				. " AND personnes_personne_id = " . $personne_id 
				. " AND fonctions_fonction_id = " . $fonction_id);
        return $result->data_seek(0);
    }
	...
	// good
	public function delete_record_4champ($table, $champ1, $champ2, $champ3, $champ4, $comID, $persID, $year, $month) {
		$table = $this->real_escape_string($table);
		$champ = $this->real_escape_string($champ1);
		$champ = $this->real_escape_string($champ2);
		$champ = $this->real_escape_string($champ3);
		$champ = $this->real_escape_string($champ4);
		$champ = $this->real_escape_string($year);
		$champ = $this->real_escape_string($month);
        $this->query("DELETE FROM " . $table . " WHERE " . $champ1 . " = " . $comID
			. " AND " . $champ2 . " = " . $persID 
			. " AND " . $champ3 . " = " . $year
			. " AND " . $champ4 . " = " . $month);
    }
	// good
	public function delete_record_2champ($table, $champ1, $champ2, $comID, $persID) {
		$table = $this->real_escape_string($table);
		$champ = $this->real_escape_string($champ1);
		$champ = $this->real_escape_string($champ2);
        $this->query("DELETE FROM " . $table . " WHERE " . $champ1 . " = " . $comID
