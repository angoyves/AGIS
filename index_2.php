...
if (!function_exists("tronque")) {
	...
	if (strlen($message) > $nb) {
		$message = substr($message, 0, -1*$nb);
		$position_espace = strpos($message, ' '); 
...
<br><form action="index_2.php" method="post">
...
?>
    ...
    <tr>
      <th align="right" scope="row">Pourcentatge :</th>
      <td colspan="3"><input name="percent" type="text">&nbsp;</td>
      <td>&nbsp;</td>
    </tr>
  </table>
</form>
...
<?php if ($totalRows_rsSelSessions == 0) { // Show if recordset not empty ?>
  ...
  <table align="center" cellpadding="0" cellspacing="0" class="std">
    ...
    <?php 
    mysql_select_db($database_MyFileConnect, $MyFileConnect);
	$query_rsCommissionsR = sprintf("SELECT commission_id, commissions.localite_id, localite_lib, commissions.commission_lib, commissions.region_id, commissions.type_commission_id FROM commissions, localites
    ...
    <tr bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
      <td colspan="9"><a href="" onClick="<?php popup($link, "1200", "700"); ?>">
	  ...
	  echo tronque($row_rsCommissionsR['commission_lib'], 15); 
	  $message = substr($row_rsCommissionsR['commission_lib'], -15);
	  echo strstr($message, 'du ');
	  //$position_espace = strpos($message, ' ').' - ' .
	  //$texte = substr($message, 0, $position_espace);
	  ...
	  echo GetRegionLibByTYpe($row_rsCommissionsR['type_commission_id']) .' '.strtoupper($row_rsCommissionsR['localite_lib']); 
        ...
        <?php if (isset($_SESSION['MM_UserGroup']) && ($_SESSION['MM_UserGroup']==1 || $_SESSION['MM_UserGroup']==8)) { ?>
          ...
          <?php } ?>      <?php //echo number_format($montant_brut1 = MinmapDB::getInstance()->get_montant_commissions($row_rsCommissionsR['commission_id'], $txtYear, $txtMonth1, $txtMonth2),0,' ',' ');?>&nbsp;
          <?php //echo number_format($montant_brut2 = MinmapDB::getInstance()->get_montant_sous_commissions($row_rsCommissionsR['commission_id'], $txtYear, $txtMonth1, $txtMonth2),0,' ',' ');?>          &nbsp;
          <?php //echo number_format($montant_brut3 = MinmapDB::getInstance()->get_montant_representants($row_rsCommissionsR['commission_id'], $txtYear, $txtMonth1, $txtMonth2, 40),0,' ',' ');?>          &nbsp;
          <?php //echo number_format($montant_brut = $montant_brut1 + $montant_brut2 + $montant_brut3,0,' ',' '); 
	  $somme_brut = $somme_brut + $montant_brut;?>&nbsp;          <?php //echo number_format($somme_retenu = $montant_brut*$Retenu ,0,' ',' '); 
	  $somme_brut = $somme_brut + $montant_brut;?>          <strong><?php //echo number_format($montant_brut - $somme_retenu ,0,' ',' '); 
	  $somme_brut = $somme_brut + $montant_brut;?></strong>&nbsp;
        <?php  if ( isset($_SESSION['MM_UserID']) && ($_SESSION['MM_UserID']==12)) {
		  $link = 'scriptReinitialiseAll.php?comID='.$row_rsCommissionsR['commission_id'].'&aID='.$txtYear.'&value=2&champ=userValidate'; ?>
        <?php } ?>&nbsp;</td>
      <?php $link = 'sample53.php?txtMonth1='. $txtMonth1 .'&txtMonth2='. $txtMonth2 .'&txtYear='. $txtYear .'&comID='. $row_rsCommissionsR['commission_id']; 
	  		$link2 = 'etat_appurements_2.php?m1ID='. $txtMonth1 .'&m2ID='. $txtMonth2 .'&aID='. $txtYear .'&comID='. $row_rsCommissionsR['commission_id'];
			$link222 = 'etat_appurements_1.php?m1ID='. $txtMonth1 .'&m2ID='. $txtMonth2 .'&aID='. $txtYear .'&comID='. $row_rsCommissionsR['commission_id'];
	  ?>
      </tr>
    <tr>
      <td colspan="9">
      <?php
	  mysql_select_db($database_MyFileConnect, $MyFileConnect);
		$query_rsSessions = sprintf("SELECT commission_id, commission_lib, type_commission_id, personnes.personne_id, personne_nom, personne_prenom, fonction_lib, nombre_jour, jour, mois, annee, montant, (nombre_jour * montant) as total, agence_cle, banque_code, agence_code, numero_compte, cle
		...
		GROUP BY personnes.personne_id, fonctions_fonction_id
		ORDER BY sessions.membres_commissions_commission_id, fonctions.fonction_id",GetSQLValueString($txtMonth1, "text"),GetSQLValueString($txtMonth2, "text"), GetSQLValueString($txtYear, "text"), GetSQLValueString($row_rsCommissionsR['commission_id'], "int"), GetSQLValueString($row_rsCommissionsR['commission_id'], "int"));
	...
	<tr>
      ...
      <tr>
		...
		?>&nbsp; </a></td>
        <td align="left" nowrap="nowrap"><?php echo $row_rsSessions['fonction_lib']; ?>&nbsp;</td>


        <td align="right" nowrap="nowrap">
			  <?php
                if (isset($row_rsSessions['type_commission_id']) && $row_rsSessions['type_commission_id'] == 6){
				$nombre_jours_year = MinmapDB::getInstance()->get_nombre_jour_sCom_by_year($row_rsCommissionsR['commission_id'], $row_rsSessions['personne_id'], $row_rsSessions['fonctions_fonction_id'], $txtMonth1, $txtMonth2, $txtYear);
				...
				$nombre_jours_year = MinmapDB::getInstance()->get_nombre_jour_by_year($row_rsCommissionsR['commission_id'], $row_rsSessions['personne_id'], $txtMonth1, $txtMonth2, $txtYear);
				if (isset($nombre_jours_year))
				{ 
					echo $nombre_jours_year;
				}
		...
		$montanSessions = GetMontantMembreSessionsSousCommission2($row_rsCommissionsR['commission_id'], $row_rsSessions['personne_id'], $row_rsSessions['fonctions_fonction_id'], $txtMonth1, $txtMonth2, $txtYear, $row_rsSessions['commission_id']);
		$montantPaye =  MinmapDB::getInstance()->get_montant_appurement_by_rib($user_rib, $txtSearch, $row_rsCommissionsR['commission_id'], $txtYear);
		} else {
		$montanSessions = GetMontantMembreSessionsCommission($row_rsCommissionsR['commission_id'], $row_rsSessions['personne_id'], $row_rsSessions['fonctions_fonction_id'], $txtMonth1, $txtMonth2, $txtYear);
		$montantPaye =  MinmapDB::getInstance()->get_montant_appurement_by_rib($user_rib, $txtSearch, $row_rsSessions['commission_id'], $txtYear);
		}
		//$montantPaye =  MinmapDB::getInstance()->get_montant_appurement_by_rib($user_rib, $row_rsCommissionsR['commission_id'], $txtYear); 
...
<tr>
    ...
    <tr>
      ...
      <td><?php echo number_format($somme_brut,0,' ',' '); $somme_total = $somme_total + $somme_brut; ?>&nbsp;</td>
      <td>reste &agrave; payer</td>
      <td><?php echo number_format($cumulResteAPayer,0,' ',' '); ?>&nbsp;</td>
    </tr>
    ...
    <tr>
      ...
      <td><strong><?php echo number_format($somme_total,0,' ',' '); ?></strong>&nbsp;</td>
      <td colspan="2">&nbsp;</td>
    </tr>
    <tr>
      <td colspan="2"><strong>INDEMNITES MINMAP</strong></td>
      <td colspan="4" align="right"></td>
      <td><strong><?php echo number_format($Indemnite_Minmap = MinmapDB::getInstance()->get_montant_indemnite_minmap($txtYear, $txtMonth1, $txtMonth2, 41),0,' ',' ');?></strong>&nbsp;</td>
      <td colspan="2">&nbsp;</td>
    </tr>
  </table>
  <?php } // Show if recordset not empty ?>
