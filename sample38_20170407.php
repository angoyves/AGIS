...
//echo $_GET['comID'].'  '. $_POST['txt_Month'].'   '. $_POST['txt_Year'];
$linkSession = "sample23.php?&comID=".$_GET['comID']."&menuID=4&action=new&lID=".$_GET['lID']."&valid=mb";

if (isset($_GET['mID']) && $_GET['mID'] != 0) {
$etat_validate = (MinmapDB::getInstance()->afficher_etat_valide($_GET['comID'], $_GET['mID'], $_GET['aID']));
$etat_controlate = (MinmapDB::getInstance()->afficher_etat_control($_GET['comID'], $_GET['mID'], $_GET['aID']));
} else {
...
$link11 = "sample42.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&comID=" . $_GET['comID']. "&menuID=" . $_GET['menuID'];
$link12 = "etat_financier_recap_pers.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&comID=" . $_GET['comID']. "&menuID=" . $_GET['menuID'];
if (isset($_GET['mID']) && $_GET['mID'] != 0) {
$link2 = "sample26.php?aID=" . $_GET['aID']. "&mID=" . $_GET['mID'] . "&comID=" . $_GET['comID'] . "&menuID=" . $_GET['menuID'] . "&lID=" . $_GET['lID'];
} else {
...
$link15 = "sample34.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&comID=" . $_GET['comID']. "&menuID=" . $_GET['menuID']."&tab=4";
$link16 = "sample36.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&menuID=" . $_GET['menuID']. "&lID=" . $_GET['lID'];
$link17 = "sample42.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&comID=" . $_GET['comID']. "&menuID=" . $_GET['menuID'];
$link18 = "sample44.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&comID=" . $_GET['comID']. "&menuID=" . $_GET['menuID'];
$link19 = "sample54.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&comID=" . $_GET['comID']. "&menuID=" . $_GET['menuID']."&tab=4";
if (isset($_GET['mID']) && $_GET['mID'] != 0) {
$link20 = "sample63.php?aID=" . $_GET['aID']. "&mID=" . $_GET['mID'] . "&comID=" . $_GET['comID'] . "&menuID=" . $_GET['menuID'] . "&pID=" . $_GET['menuID'];
} else {
$link20 = "sample63.php?aID=" . $_POST['txt_Year']. "&mID=" . $_POST['txt_Month'] . "&comID=" . $_GET['comID'] . "&menuID=" . $_GET['menuID'] . "&pID=" . $_GET['menuID'];
}
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
if (isset($_GET['mID']) && $_GET['mID'] != 0 ){
$query_rsJourMois = sprintf("SELECT * FROM mois WHERE mois_id = %s", GetSQLValueString($_GET['mID'], "text"));
} else {
...
AND mois = %s 
AND annee = %s 
ORDER BY fonctions.fonction_id", GetSQLValueString($_GET['comID'], "int"),GetSQLValueString($_GET['mID'], "text"),GetSQLValueString($_GET['aID'], "text"));
} else {
$query_rsSessions = sprintf("SELECT commission_id, personnes.personne_id, personne_nom, fonction_lib, nombre_jour, jour, mois, annee, montant, (nombre_jour * montant) as total
FROM commissions, membres, personnes,  fonctions, sessions
WHERE membres.commissions_commission_id = commissions.commission_id
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
if (isset($_GET['aID']) && $_GET['aID'] != 0 ){
$query_rsDossiers_traites = sprintf("SELECT * FROM dossier_traites WHERE dossiers_commission_id = %s AND mois = %s AND annee = %s", GetSQLValueString($colname_rsDossiers_traites, "int"), GetSQLValueString($_GET['mID'], "text"),GetSQLValueString($_GET['aID'], "text"));
} else {
        ...
        <table align="left" class="std2">
          <tr valign="baseline">
            ...
            <td><select name="txt_Month" id="txt_Month">
              <option value="" <?php if (!(strcmp("", $_GET['mID']))) {echo "selected=\"selected\"";} ?>>:: Select...</option>
              <?php do {  ?>
              <?php if (isset($_GET['mID']) && $_GET['mID'] != 0 ) { ?>
              <option value="<?php echo $row_rsMois['mois_id']?>"<?php if (!(strcmp($row_rsMois['mois_id'], $_GET['mID']))) {echo "selected=\"selected\"";} ?>> <?php echo ucfirst($row_rsMois['lib_mois']) ?></option>
              <?php } else { ?>
       ...
       if ($moisIsEmpty) { ?>
          ...
          <tr valign="baseline">
            ...
            <td><select name="txt_Year" id="txt_Year">
              <option value="" <?php if (!(strcmp("", $_GET['aID']))) {echo "selected=\"selected\"";} ?>>:: Select...</option>
              <?php
do {  
?>
              <?php if (isset($_GET['aID']) && $_GET['aID'] != 0) { ?>
              <option value="<?php echo $row_rsSelYear['lib_annee']?>"<?php if (!(strcmp($row_rsSelYear['lib_annee'], $_GET['aID']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsSelYear['lib_annee']?></option>
              <?php } else { ?>
...
?>
    ...
    <tr>
      <td><input type="submit" value="Rechercher..." />
        <a href="<?php echo $linkSession ?>">Ajouter une session</a></td>
    </tr>
  </table>
</form>
...
<table>
  <tr>
    ...
    <?php } ?>&nbsp;&nbsp;
      ...
      <?php if (isset($etat_validate) && $etat_validate!=2) { ?>
      <a href="<?php echo $link2 . "&valid=mb" ?>"><img src="images/img/b_edit.png" width="16" height="16" /> Modifier la session</a>&nbsp;&nbsp; 
      <?php } ?>
      <a href="<?php echo $link10."&valid=mb" ?>"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/>Ajouter une nouvelle session à la commission</a>&nbsp;&nbsp; <a href="#"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/>Supprimer la session de la commission</a></BR></td>
  </tr>
</table>
<?php } ?>
...
<table border="1" align="center" class="std">
  ...
  <?php } ?>  
    ...
    <th nowrap>&nbsp;</th>
    <?php } ?>
    <th nowrap>Nom et Prenom</th>
    <th nowrap>Fonction</th>
    <?php $counter=0; do  { $counter++; ?>
    <th nowrap><?php echo $counter ?>&nbsp;</th>
  ...
  <tr>
    ...
    <td nowrap="nowrap">
        <a href="<?php echo $link5 . $row_rsSessions['personne_id']; ?>" onClick="return confirm('Etes vous sur de vouloir supprimer 																																																				cette sessions ?');"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/></a><a href="maj_personnes.php?recordID=<?php echo $row_rsSessions['membres_personnes_personne_id']; ?>&amp;map=personnes"></a>
        </td><?php } ?>
    ...
    <tr>
      ...
      <table align="left">
        <tr>
          ...
          <?php if (isset($etat_validate) && $etat_validate!=2) { ?> 
          <a href="<?php echo $link16 . "&valid=mb&comID=" . $_GET['comID'] . "&sCom=" . $row_rsSousCommission['commission_id'] ?>"><img src="images/img/b_edit.png" width="16" height="16" /> Modifier</a>&nbsp;&nbsp;
          <?php } ?>
    ...
    <tr>
      ...
      <td><a href="#">
        <?php $com_ID = $row_rsSousCommission['commission_id'];
	  		echo MinmapDB::getInstance()->get_dossier_ref_by_value($com_ID, $_POST['txt_Month'], $_POST['txt_Year']); ?>
        &nbsp;</a></td>
      <td valign="top">
	  <?php
	  	$colname_rsMembres = "-1";
...
<table>
  <tr>
    ...
    <!--<form action="scriptValidate.php" method="post">-->
		...
		$link = 'scriptValidate.php?comID='.$_GET['comID'].'&mID='.$_POST['txt_Month'].'&aID='.$_POST['txt_Year']; ?>
