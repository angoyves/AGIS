...
$link13 = "print_payment_report.php?aID=" . $_POST['txtYear']. "&mID1=" . $_POST['txtMonth1'] . "&mID2=" . $_POST['txtMonth2'] . "&gID=" . $_POST['txtGroup']. "&menuID=" . $_GET['menuID']."&tab=3";
...
$query_rsSousGroupe = sprintf("SELECT groupes.groupe_id, groupe_lib, sous_groupes.sous_groupe_id, sous_groupe_lib, sous_groupes.pourcentage as taux, count(personne_id) Nombres FROM sous_groupes, personnes, groupes
...
group by sous_groupes.sous_groupe_id",GetSQLValueString($colname_rstxtGroup, "int"));
...
$query_rsSousGroupe = "SELECT groupes.groupe_id, groupe_lib, sous_groupes.sous_groupe_id, sous_groupe_lib, sous_groupes.pourcentage as taux, count(personne_id) Nombres
FROM sous_groupes, personnes, groupes
WHERE personnes.sous_groupe_id = sous_groupes.sous_groupe_id
AND sous_groupes.groupe_id = groupes.groupe_id
AND personnes.type_personne_id between 1 and 2
AND personnes.display = 1
group by sous_groupes.sous_groupe_id";
$rsSousGroupe = mysql_query($query_rsSousGroupe, $MyFileConnect) or die(mysql_error());
$row_rsSousGroupe = mysql_fetch_assoc($rsSousGroupe);
$totalRows_rsSousGroupe = mysql_num_rows($rsSousGroupe);
}
...
<title>PROMAGAR MINFI : Accueil</title>
<!-- InstanceEndEditable -->
...
?>
    ...
    <tr>
      <td colspan="4"><input type="hidden" name="MM_update" value="form1" />
      &nbsp;<?php if (isset($_POST['txtMonth1']) && isset($_POST['txtMonth2'])) { ?>
...
<p>&nbsp;
<p>
<strong><h1>indemnite forfitaire speciale par sous-groupe</h1></strong>
<table width="70%" border="1" align="center" class="std">
  <tr>
    <th nowrap>N&deg;</th>
    <th nowrap>Groupe</th>
