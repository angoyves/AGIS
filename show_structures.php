...
$otherPageNew = 'new_structures.php';

$maxRows_rsStructures = 20;
...
$colname_rsStructures = "a";
if (isset($_POST['txtSearch'])) {
  $colname_rsStructures = $_POST['txtSearch'];
}
$colname2_rsStructures = "a";
if (isset($_POST['txtSearch'])) {
  $colname2_rsStructures = $_POST['txtSearch'];
}
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsStructures = sprintf("SELECT * FROM structures, type_structure, localites WHERE structures.type_structure_id = type_structure.type_structure_id AND structures.localite_id = localites.localite_id   AND (structure_lib like %s OR localite_lib like %s) AND structures.display =1 ", GetSQLValueString("%" . $colname_rsStructures . "%", "text"),GetSQLValueString("%" . $colname2_rsStructures . "%", "text"));
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_Recordset1 = "SELECT * FROM menus WHERE display = '1' ORDER BY menu_id ASC";
...
<form method="post" action="" >    
    <table border="0">
      <tr>
        <td colspan="2">
          <a href="#" onclick="<?php popup("new_structure.php", "610", "500"); ?>"><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/>Ajouter une structure</a>
        </td>
        <td></td>
      </tr>
      <tr>
        <td align="center" valign="middle">Rechecher :</td>
        <td align="center" valign="middle"><input name="txtSearch" type="text"></td>
        <td align="center" valign="middle"><input name="button" type="submit" value="Rechercher..."></td>
      </tr>
      <tr>
        <td>&nbsp;</td>
        <td></td>
...
<table border="1" align="center" class="std">
  ...
  <?php do { ?>
    <tr>
      <td><a href="detail_structure.php?recordID=<?php echo $row_rsStructures['structure_id']; ?>"> <?php echo $row_rsStructures['structure_id']; ?>&nbsp; </a></td>
      <td><?php echo $row_rsStructures['structure_lib']; ?>&nbsp; </td>
      <td><?php echo $row_rsStructures['code_structure']; ?>&nbsp; </td>
      <td><?php echo $row_rsStructures['type_structure_lib']; ?>&nbsp; </td>
      <td><?php echo $row_rsStructures['localite_lib']; ?>&nbsp; </td>
<td><?php if (isset($row_rsBanques['display']) && $row_rsBanques['display'] == '1') { ?>
              <a href="change_etat.php?recordID=<?php echo $row_rsBanques['banque_code']; ?>&&map=agences&&mapid=agence_id&&action=desactive"><img src="images/img/s_okay.png" alt="" width="16" height="16" align="absmiddle"/></a>
              <?php } else { ?>
              <a href="change_etat.php?recordID=<?php echo $row_rsBanques['banque_code']; ?>&&map=agences&&mapid=agence_id&&action=active"><img src="images/img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a>
              <?php } ?></td>
            <td><a href="edit_agences.php?recordID=<?php echo $row_rsBanques['banque_code']; ?>&&map=agences"><img src="images/img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
            <td><a href="delete.php?recordID=<?php echo $row_rsBanques['banque_code']; ?>&&map=agences"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
            <td><a href="detail_agences.php?recordID=<?php echo $row_rsBanques['banque_code']; ?>"><img src="images/img/b_view.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
    </tr>
    <?php } while ($row_rsStructures = mysql_fetch_assoc($rsStructures)); ?>
</table>
<br />
<table border="0">
  <tr>
    <td><?php if ($pageNum_rsStructures > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_rsStructures=%d%s", $currentPage, 0, $queryString_rsStructures); ?>">Premier</a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_rsStructures > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_rsStructures=%d%s", $currentPage, max(0, $pageNum_rsStructures - 1), $queryString_rsStructures); ?>">Pr&eacute;c&eacute;dent</a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_rsStructures < $totalPages_rsStructures) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_rsStructures=%d%s", $currentPage, min($totalPages_rsStructures, $pageNum_rsStructures + 1), $queryString_rsStructures); ?>">Suivant</a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_rsStructures < $totalPages_rsStructures) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_rsStructures=%d%s", $currentPage, $totalPages_rsStructures, $queryString_rsStructures); ?>">Dernier</a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
Enregistrements <?php echo ($startRow_rsStructures + 1) ?> à <?php echo min($startRow_rsStructures + $maxRows_rsStructures, $totalRows_rsStructures) ?> sur <?php echo $totalRows_rsStructures ?>
<!-- InstanceEndEditable -->
    <!--div class="bloc express">
				<h4>Informations express</h4>
				<p><span>9 mars 2009</span><br>
