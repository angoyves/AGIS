...
$colname_rsAddMembre = "-1";
if (isset($_GET['comID'])) {
  $colname_rsAddMembre = $_GET['comID'];
}
...
$count = 0; do { $count++;
	if (isset($_POST['membre_personne_id'.$count]) && $_POST['membre_personne_id'.$count] != "" && isset($_POST['days'.$count]) && $_POST['days'.$count] != null){
	$insertSQL = sprintf("INSERT INTO sessions (membres_commissions_commission_id, dossiers_dossier_id, nombre_dossier, nombre_jour, mois, annee, membres_personnes_personne_id, membres_fonctions_fonction_id, jour, dateCreation, display, user_id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
						   ...
						   GetSQLValueString(count($_POST['days'.$count]), "int"),
						   GetSQLValueString($_GET['mID'], "text"),
						   GetSQLValueString($_GET['aID'], "text"),
						   GetSQLValueString($_POST['membre_personne_id'.$count], "int"),
						   GetSQLValueString($_POST['membre_fonction_id'.$count], "int"),
						   GetSQLValueString(implode( "**", $_POST['days'.$count] ), "text"),
	...
	$updateSQL = sprintf("UPDATE membres SET  display='1' WHERE commissions_commission_id=%s AND fonctions_fonction_id=%s AND personnes_personne_id=%s",
						   GetSQLValueString($_POST['commission_id'], "int"),
						   GetSQLValueString($_POST['membre_fonction_id'.$count], "int"),
						   GetSQLValueString($_POST['membre_personne_id'.$count], "int"));
...
} while ($count < $_POST['nombre_membres']);
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsAddMembre = sprintf("SELECT commission_id, commissions_commission_id, personne_nom, personne_id, personne_prenom, fonction_lib, checboxName, position, fonctions_fonction_id FROM commissions, membres, fonctions, personnes WHERE membres.commissions_commission_id = commissions.commission_id   AND fonctions_fonction_id = fonctions.fonction_id   AND personnes_personne_id = personne_id  AND membres.fonctions_fonction_id BETWEEN 1 AND 3 AND personnes.display = 1  AND membres.display = 2 AND commissions_commission_id = %s ORDER BY personne_nom ", GetSQLValueString($colname_rsAddMembre, "int"));
$rsAddMembre = mysql_query($query_rsAddMembre, $MyFileConnect) or die(mysql_error());
$row_rsAddMembre = mysql_fetch_assoc($rsAddMembre);
$totalRows_rsAddMembre = mysql_num_rows($rsAddMembre);
?>
      ...
      <td><table border="0">
        <tr>
          ...
          <td><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle" />&nbsp;
              <?php           
			$showGoTo = "choose_membre.php?uID=".$_SESSION['MM_Username'];
			if (isset($_SERVER['QUERY_STRING'])) {
				$showGoTo .= (strpos($showGoTo, '?')) ? "&" : "?";
				$showGoTo .= $_SERVER['QUERY_STRING'];
			}
	 	  ?>
        <a href="#" onclick="<?php popup($showGoTo, "710", "450"); ?>">Ajouter  un nouveau membre &agrave; la commission...</a>&nbsp;&nbsp;</td>
        </tr>
      </table></td>
    </tr>
    <tr>
...
// debut
 foreach(GetValueList($row_rsJourMois['mois_id']) as $value){ ?>
	...
	?>
       ...
       <?php if ($totalRows_rsAddMembre > 0) { // Show if recordset not empty ?>
        ...
        <table border="1" class="std">
          ...
          <?php $count = 0; do { $count++; ?>
            <tr>
              <td><a href="show_personnes.php?txtChamp=personne_id&amp;txtSearch=<?php echo $row_rsAddMembre['personne_id']; ?>&amp;menuID=2&amp;action=upd"> <?php echo $count; ?></a>&nbsp; </td>
              <td><?php echo strtoupper($row_rsAddMembre['personne_nom'] . " " . $row_rsAddMembre['personne_prenom']); ?>
                <input type="hidden" name="<?php echo "membre_personne_id".$count; ?>" value="<?php echo $row_rsAddMembre['personne_id']; ?>" />
                &nbsp; </td>
              <td><?php echo strtoupper($row_rsAddMembre['fonction_lib']); ?><input type="hidden" name="<?php echo "membre_fonction_id".$count; ?>" value="<?php echo $row_rsAddMembre['fonctions_fonction_id']; ?>" />
                &nbsp; </td>
              <?php //$counter = 0; do { $counter++;
			foreach(GetValueList($_GET['mID']) as $value){
			switch($count){
			case $count :
			$liste = $_POST['days'.$count];
		  ?>
              <td><input type="checkbox" name="<?php echo "days". $count . "[]" ?>" id="<?php echo "days". $count . $value ?>" value="<?php echo $value ?>"  
          <?php echo ((in_array($value, $liste))? "checked='checked'":"") ?> /></td>
              <?php break; } 
		  } ?>
            <?php //} while($counter < $row_rsJourMois['nbre_jour']); ?>
<td><a href="activ_rep2.php?aID=<?php echo $_GET['aID']; ?>&amp;mID=<?php echo $_GET['mID']; ?>&amp;comID=<?php echo $_GET['comID']; ?>&amp;perID=<?php echo $row_rsAddMembre['personne_id'];?>&amp;action=desactive&amp;menuID=<?php echo $_GET['menuID']; ?>&page=sample26&foncID=<?php echo $row_rsAddMembre['fonctions_fonction_id']; ?>"><img src="images/img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
</tr>
<input type="hidden" name="nombre_membres" id="nombre_membres" value="<?php echo $count ?>"/>
          <?php } while ($row_rsAddMembre = mysql_fetch_assoc($rsAddMembre)); ?>
      </table>
       <?php } // Show if recordset not empty ?>
      ...
      <table border="1" class="std">
          ...
          <?php echo ((in_array($value, $liste))? "checked='checked'":"") ?> /></td>
            ...
            <?php //} while($counter < $row_rsJourMois['nbre_jour']); ?>
<td><a href="activ_rep2.php?aID=<?php echo $_GET['aID']; ?>&amp;mID=<?php echo $_GET['mID']; ?>&amp;comID=<?php echo $_GET['comID']; ?>&amp;perID=<?php echo $row_rsRepresentant['personne_id'];?>&amp;action=desactive&amp;menuID=<?php echo $_GET['menuID']; ?>&page=sample26&fonId=<?php echo $row_rsRepresentant['fonctions_fonction_id']; ?>"><img src="images/img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
</tr>
