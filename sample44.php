...
ORDER BY personnes.personne_nom",  GetSQLValueString($colname_rsSessions, "int"),GetSQLValueString($colname_rsSessions2, "text"));
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsSessionRepresentant = sprintf("SELECT commission_id, personnes.personne_id, personne_nom, personne_prenom, fonction_lib, code_structure, sum(nombre_jour) as nombre_jour, jour, mois, annee, montant, (nombre_jour * montant) as total, agence_cle, banque_code, agence_code, numero_compte
FROM commissions, membres, personnes,  fonctions, sessions, rib, structures 
WHERE membres.commissions_commission_id = commissions.commission_id
AND membres.personnes_personne_id = personnes.personne_id
...
AND annee = %s
GROUP BY personnes.personne_nom 
ORDER BY personnes.personne_nom", GetSQLValueString($colname_rsSessionRepresentant, "int"),GetSQLValueString($colname1_rsSessionRepresentant, "int"));
	...
	<tr>
      ...
      <tr>
        ...
        <td align="right" nowrap="nowrap"><?php 
				...
				{ 
					echo number_format($nombre_jours * $row_rsSessions['montant'],0,' ',' ');
...
<tr>
        <td nowrap="nowrap">&nbsp; </td>
        <td align="left" nowrap="nowrap"><a href="#" onclick="<?php popup($showGoToPersonnes1, "700", "400"); ?>">&nbsp; </a></td>
      ...
      <tr>
        <td colspan="17" nowrap="nowrap">&nbsp;</td>
  ...
  <table width="50%" border="1" align="center" class="print">
    <tr>
      ...
      <?php }while ($counter < 12);?>
      <th nowrap="nowrap">Total Sessions</th>
      <th>RECAP</th>
      </tr>
    ...
    <tr>
      ...
      <td align="right" nowrap="nowrap"><?php /*echo $_GET['comID'] ."-" . $row_rsSessions_sCom['personne_id'] ."-" . $row_rsSessions_sCom['fonction_id'] ."-" . $count ."-" .$_GET['aID'];/*
			...
			<?php 
				...
				$nombre_jours = MinmapDB::getInstance()->get_nombre_jour_sCom_by_values($_GET['comID'], $row_rsSessions_sCom['personne_id'], $count, $_GET['aID']);
				if (isset($nombre_jours))
				{ 
					echo $nombre_jours;
					$sommer = $sommer + $nombre_jours;
      ...
      <td align="right" nowrap="nowrap"><strong><?php $somme2 = $row_rsSessions_sCom['montant']*$sommer; echo number_format($row_rsSessions_sCom['total'],0,' ',' '); ?></strong>
          ...
          <tr>
            ...
            <td align="right" nowrap="nowrap">
              <?php 
				...
				if (isset($nombre_jour)){ 
				echo number_format(($row_rsSessionRepresentant['montant']*$nombre_jour),0,' ',' ');; 
				$sommes = $sommes + $nombre_jour;
				} else {echo 0;}		
			?>
            &nbsp;</td>
            <?php } while($count<12) ?>
