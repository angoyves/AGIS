...
$query_rsSessions = "SELECT membres_personnes_personne_id, personne_nom, personne_prenom, membres_commissions_commission_id,  membres_fonctions_fonction_id, fonction_lib, lib_nature, type_commission_lib, localite_lib FROM sessions, personnes, commissions, fonctions, natures, type_commissions, localites WHERE sessions.membres_personnes_personne_id = personnes.personne_id AND sessions.membres_commissions_commission_id = commissions.commission_id AND sessions.membres_fonctions_fonction_id = fonctions.fonction_id  AND commissions.nature_id = natures.nature_id  AND commissions.type_commission_id = type_commissions.type_commission_id  AND commissions.localite_id = localites.localite_id";
...
$totalPages_rsSessions = ceil($totalRows_rsSessions/$maxRows_rsSessions)-1;$maxRows_rsSessions = 10;
$pageNum_rsSessions = 0;
if (isset($_GET['pageNum_rsSessions'])) {
  $pageNum_rsSessions = $_GET['pageNum_rsSessions'];
}
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_rsSessions = "SELECT membres_personnes_personne_id, personne_nom, personne_prenom, membres_commissions_commission_id,  membres_fonctions_fonction_id, fonction_lib, lib_nature, type_commission_lib, localite_lib, jour, mois, annee FROM sessions, personnes, commissions, fonctions, natures, type_commissions, localites WHERE sessions.membres_personnes_personne_id = personnes.personne_id AND sessions.membres_commissions_commission_id = commissions.commission_id AND sessions.membres_fonctions_fonction_id = fonctions.fonction_id  AND commissions.nature_id = natures.nature_id  AND commissions.type_commission_id = type_commissions.type_commission_id  AND commissions.localite_id = localites.localite_id";
$query_limit_rsSessions = sprintf("%s LIMIT %d, %d", $query_rsSessions, $startRow_rsSessions, $maxRows_rsSessions);
$rsSessions = mysql_query($query_limit_rsSessions, $MyFileConnect) or die(mysql_error());
$row_rsSessions = mysql_fetch_assoc($rsSessions);
...
<table border="1" align="center" class="std">
  <tr>
    ...
    <th>Libelle type commission</th>
    <th>Libelle localite</th>
  </tr>
  <?php do { ?>
    <tr>
      <td><?php echo $row_rsSessions['membres_personnes_personne_id']; ?>&nbsp; </td>
      <td><?php echo $row_rsSessions['personne_nom']; ?>&nbsp; </td>
      <td><?php echo $row_rsSessions['personne_prenom']; ?>&nbsp; </td>
      <td><a href="to_delete_session.php?recordID=<?php echo $row_rsSessions['membres_commissions_commission_id']; ?>"> <?php echo $row_rsSessions['membres_commissions_commission_id']; ?>&nbsp; </a></td>
      <td><?php
					$countDoc = 0;
					foreach(explode("**", $row_rsSessions['jour']) as $val){
						$countDoc++; 
						$val!= "" ? (print "<li>".$val."-".$row_rsSessions['mois']."-".$row_rsSessions['annee']."<br />") : $countDoc--; 
						
					}
				?>&nbsp;</td>
      <td><?php echo $row_rsSessions['fonction_lib']; ?>&nbsp; </td>
      <td><?php echo $row_rsSessions['lib_nature']; ?>&nbsp; </td>
      <td><?php echo $row_rsSessions['type_commission_lib']; ?>&nbsp; </td>
...
<table border="0">
  <tr>
    <td><?php if ($pageNum_rsSessions > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_rsSessions=%d%s", $currentPage, 0, $queryString_rsSessions); ?>">First</a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_rsSessions > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_rsSessions=%d%s", $currentPage, max(0, $pageNum_rsSessions - 1), $queryString_rsSessions); ?>">Previous</a>
        <?php } // Show if not first page ?></td>
    <td><?php if ($pageNum_rsSessions < $totalPages_rsSessions) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_rsSessions=%d%s", $currentPage, min($totalPages_rsSessions, $pageNum_rsSessions + 1), $queryString_rsSessions); ?>">Next</a>
        <?php } // Show if not last page ?></td>
    <td><?php if ($pageNum_rsSessions < $totalPages_rsSessions) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_rsSessions=%d%s", $currentPage, $totalPages_rsSessions, $queryString_rsSessions); ?>">Last</a>
        <?php } // Show if not last page ?></td>
  </tr>
</table>
Records <?php echo ($startRow_rsSessions + 1) ?> to <?php echo min($startRow_rsSessions + $maxRows_rsSessions, $totalRows_rsSessions) ?> of <?php echo $totalRows_rsSessions ?>
<!-- InstanceEndEditable -->
    <!--div class="bloc express">
				<h4>Informations express</h4>
				<p><span>9 mars 2009</span><br>
