...
<table>
  <tr>
    ...
    <!--<form action="scriptValidate.php" method="post">-->
    	...
    	<?php 
$link = 'scriptReinitialiseAll.php?comID='.$commission_id.'&mID='.$txt_Month.'&aID='.$txt_Year.'&value=2&champ=userValidate'; ?>
    	<input name="BtnValider" type="submit" value="Tous Valider..." onclick="<?php popup($link, "1", "1"); ?>">
		<?php  if ( isset($etat_validate) && $etat_validate == 0 && isset($_SESSION['MM_UserGroup']) && ($_SESSION['MM_UserGroup']==1 || $_SESSION['MM_UserGroup']==6)) {
		$link = 'scriptControlate2.php?comID='.$commission_id.'&mID='.$txt_Month.'&aID='.$txt_Year.'&value=1&champ=userValidate'; ?>
    	<input name="BtnValider" type="submit" value="Valider la Saisie de l'etat de la  commission" onclick="<?php popup($link, "1", "1"); ?>">
      ...
      <?php } elseif (isset($etat_validate) && $etat_validate=='2' && isset($_SESSION['MM_UserGroup']) && ($_SESSION['MM_UserGroup']==1 || $_SESSION['MM_UserGroup']==6)) { ?>
        ...
        <?php } ?>
        <?php if (isset($etat_validate) && ($etat_validate == 1 || $etat_validate == 2)  && isset($_SESSION['MM_UserGroup']) && ($_SESSION['MM_UserGroup']==1)) { 
		$linkage = 'scriptControlate2.php?comID='.$commission_id.'&mID='.$txt_Month.'&aID='.$txt_Year.'&value=0&champ=userValidate'; ?>
        <input name="BtnValider" type="submit" value="Reinitialiser la session" onclick="<?php popup($linkage, "1", "1"); ?>">
        <?php } else { ?>
			<input name="BtnValider" type="button" value="Reinitialiser la session" disabled>
		<?php } ?>
    <!--</form>--><!--Etat Saisi par : <?php //echo $row_rsSessions['user_id']; ?>--><?php //echo $etat_validate; ?></td>
  </tr>
</table>
...
<h1>Sous Commissions</h1>
  <table width="60%" border="1" align="center" class="std">
    ...
    <tr>
      ...
      <table align="left">
        <tr>
          ...
          <?php } ?>
          <a href="<?php 
		  $deleteGoTo = "delete_sca.php?comID=".$row_rsSousCommission['commission_id'];
		  if (isset($_SERVER['QUERY_STRING'])) {
		  ...
		  //echo $deleteGoTo ?>"><?php //echo $deleteGoTo ?><img src="images/img/b_snewtbl.png" alt="" width="16" height="16" align="absmiddle"/>Ajouter une session</a>&nbsp;&nbsp;<!--<a href="#"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/>Supprimer la session</a>--></td>
          </tr>
      </table>
      <?php } ?>
      </td>
    ...
    <tr>
      <td align="center" valign="middle">&nbsp; <a href="<?php 
	  $deleteGoTo = "delete_sca.php?comscaID=".$row_rsSousCommission['commission_id']."&mID=".$txt_Month."&aID=".$txt_Year;
		  if (isset($_SERVER['QUERY_STRING'])) {
			$deleteGoTo .= (strpos($deleteGoTo, '?')) ? "&" : "?";
			$deleteGoTo .= $_SERVER['QUERY_STRING'];
		  }		  
		  ...
		  echo $deleteGoTo; ?>"><img src="images/img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/><?php //echo $row_rsSousCommission['commission_id']; ?><?php //echo $count; ?></a><br />
          
          <?php
          if (isset($txt_Month) && $txt_Month != 0) {
		  $etat_validate = (MinmapDB::getInstance()->afficher_etat_valide2($commission_id, $txt_Month, $txt_Year, $row_rsSousCommission['commission_id']));
		  $etat_controlate = (MinmapDB::getInstance()->afficher_etat_control($commission_id, $txt_Month, $txt_Year));
			} ?>
      ...
      <?php 
	  $dossierID = MinmapDB::getInstance()->get_dossier_id_by_value($row_rsSousCommission['commission_id'], $txt_Month, $txt_Year);
	  //$showGoTextModif = "modif_dossiers.php?dosID=". $dossierID;
	  $showGoTextModif = "mod_dossca.php?dosID=". $dossierID; ?>
	...
	?>
        <table width="100%" align="left" id="*">
          ...
          <?php } ?>
              ...
              <td align="right" nowrap><a href="#" onclick="<?php 
			  $updateGoTo = "mod_montantsca.php?persID=".$row_rsMembres['personnes_personne_id'] ."&comID=". $row_rsSousCommission['commission_id']."&foncID=".$row_rsMembres['fonctions_fonction_id'];
			  popup($updateGoTo, "500", "200"); ?>"><?php echo number_format($row_rsMembres['montant'],0,' ',' '); $Montant_global = $Montant_global + $row_rsMembres['montant'];?><img src="images/img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a>&nbsp; </td>
            </tr>
