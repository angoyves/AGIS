...
<form id="form1" name="form1" method="get" action="sample93.php?col=<?php $_GET['txtChamp'] ?>&txt=<?php $_GET['txtSearch'] ?>">
  <table border="0" class="std">
    <tr>
      ...
      <td><select name="txtChamp" id="txtChamp">
        ...
        <option value="personne_matricule" <?php if (!(strcmp("personne_matricule", $_GET['txtChamp']))) {echo "selected=\"selected\"";} ?>>Matricule</option>
        <option value="personne_telephone" <?php if (!(strcmp("personne_telephone", $_GET['txtChamp']))) {echo "selected=\"selected\"";} ?>>N° Téléphone</option>
        <option value="personne_grade" <?php if (!(strcmp("personne_grade", $_GET['txtChamp']))) {echo "selected=\"selected\"";} ?>>Grade</option>
      </select></td>
    ...
    <tr>
      <td colspan="4"><?php echo $maxRows_rsAffichePerson; ?>&nbsp;</td>
    </tr>
    <tr>
...
<h1>
  ...
  <table width="75%" border="1" align="center" class="std">
    ...
    <tr bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
      <td colspan="2"><?php echo $counter ?>
        <?php           
	$showGoToPersonne = "show_rib.php?recordID=". $row_rsAffichePerson['personne_id'];
	...
	$showGoToPersonnes2 = "sample43.php?recordID=". $row_rsAffichePerson['personne_id'];
	$showGoToMonth = "sample94.php?pID=".$row_rsAffichePerson['personne_id'];
/*  if (isset($_SERVER['QUERY_STRING'])) {
...
?>      </td>
      ...
      <td nowrap="nowrap"> 
      <a href="#" onclick="<?php popup($showGoToPersonnes2, "700", "200"); ?>">
        <?php if (isset($_GET['txtSearch']) && $_GET['txtSearch'] != "" && ((isset($_GET['txtChamp']) && $_GET['txtChamp'] == "personne_nom") || (isset($_GET['txtChamp']) && $_GET['txtChamp'] == "personne_prenom"))) {echo str_ireplace($_GET['txtSearch'],'<span style="background-color:yellow">'. strtolower($_GET['txtSearch']) .'</span>',strtoupper(htmlentities($row_rsAffichePerson['personne_nom'] . " " . ucfirst($row_rsAffichePerson['personne_prenom']))));
	  } else { echo strtoupper(htmlentities($row_rsAffichePerson['personne_nom'] . " " . ucfirst($row_rsAffichePerson['personne_prenom'])));} ?>
        &nbsp; </a></td>
      <td>
        <?php if (isset($_GET['txtSearch']) && $_GET['txtSearch'] != "" && isset($_GET['txtChamp']) && $_GET['txtChamp'] == "personne_grade") {echo str_ireplace($_GET['txtSearch'],'<span style="background-color:yellow">'. $_GET['txtSearch'] .'</span>',htmlentities($row_rsAffichePerson['personne_grade']));
	  } else { echo htmlentities($row_rsAffichePerson['personne_grade']);} ?>&nbsp; </td>
      <td nowrap><a href="#" onclick="<?php popup($showGoToPersonnes1, "700", "400"); ?>"><?php $user_rib = MinmapDB::getInstance()->get_user_rib_by_id($row_rsAffichePerson['personne_id']);
	  if (isset($user_rib) && $user_rib <> 'xxxxxxxxxxxxxxxxxxxxxxx'){
		  echo $user_rib; }else{ echo 'Cliquez pour saisir le RIB'; } ?><?php /*echo strtoupper($row_rsAffichePerson['banque_code'].'-'.$row_rsAffichePerson['agence_code'].'-'.$row_rsAffichePerson['numero_compte'].'-'.$row_rsAffichePerson['cle']); */?></a>&nbsp;</td>
      <td>
      <?php if (isset($_GET['txtSearch']) && $_GET['txtSearch'] != "" && isset($_GET['txtChamp']) && $_GET['txtChamp'] == "personne_telephone") {echo str_ireplace($_GET['txtSearch'],'<span style="background-color:yellow">'. strtoupper($_GET['txtSearch']) .'</span>',strtoupper(htmlentities($row_rsAffichePerson['personne_telephone'])));
	  } else { echo strtoupper(htmlentities($row_rsAffichePerson['personne_telephone']));} ?>&nbsp;</td>
      <td align="center"><?php if (isset($row_rsAffichePerson['add_commission']) && $row_rsAffichePerson['add_commission'] == 2 ){ ?>
        <img src="img/b_usrlist.png" alt="" width="16" height="16" align="absmiddle"/>
        <?php } ?>&nbsp; </td>
      <td align="center"><?php if (isset($row_rsAffichePerson['display']) && $row_rsAffichePerson['display'] == '1') { ?>
        <a href="change_etat_index.php?recordID=<?php echo $row_rsAffichePerson['personne_id']; ?>&map=personnes&mapid=personne_id&action=desactive&menuID=<?php echo $_GET['menuID']; ?>"><img src="img/s_okay.png" alt="" width="16" height="16" align="absmiddle"/></a>
        <?php } else { ?>
        <a href="change_etat_index.php?recordID=<?php echo $row_rsAffichePerson['personne_id']; ?>&map=personnes&mapid=personne_id&action=active&menuID=<?php echo $_GET['menuID']; ?>"><img src="img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a>
      <?php } ?></td>
	  <?php $user_group = MinmapDB::getInstance()->get_user_groupe_by_name($_SESSION['MM_Username']);
		if (isset($_SESSION['MM_UserGroup']) && $_SESSION['MM_UserGroup'] == '1'){ ?>
      <td align="center"><a href="maj_personnes.php?recordID=<?php echo $row_rsAffichePerson['personne_id']; ?>&&map=personnes&menuID=<?php echo $_GET['menuID']; ?>"><img src="img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a></td> 
      <td align="center"><a href="del_personnes.php?recordID=<?php echo $row_rsAffichePerson['personne_id']; ?>&menuID=<?php echo $_GET['menuID']; ?>" onClick="return confirm('Etes vous sur de vouloir supprimer cet enregistrement?');" ><img src="img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
      <?php } ?>
      <td align="center"><a href="#" onClick="<?php popup($showGoToMonth, "700", "400"); ?>"><img src="img/b_view.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
...
<table width="570" border="0" cellpadding="0" cellspacing="0" class="std">
  <tr class="TabAloneWhite">
    <th width="494" height="18">Résultat de la recherche</th>
  </tr>
  <tr bgcolor="#FFFFFF">
    <td height="36"><img src="img/s_attention.png" width="16" height="16" /> Aucun enregistrement trouvé pour votre recherche &quot; <strong> <?php echo $_GET['txtSearch']; ?></strong> &quot; dans <strong> <?php echo $_GET['txtChamp']; ?></strong></td>
  </tr>
  <tr bgcolor="#FFFFFF">
    <td height="36"><p>Saisissez à nouveau votre recherche...</p></td>
  </tr>
</table>
  <?php }// Show if recordset not empty ?>
<br />

<?php if (isset($_GET['txtChamp']) && $_GET['txtChamp'] != "" && isset($_GET['txtSearch']) && $_GET['txtSearch'] != "" ) { 
 echo $totalRows_rsAffichePerson . '  Enregistrements Total'; } else { 
?>
...
<?php if ($_SESSION['MM_UserGroup']==1) { // Show if recordset not empty ?>
<h1>Personnes à l'etat desactivé</h1>
...
<table width="75%" border="1" align="center" class="std">
  ...
  <?php $counter=0; do  { $counter++; ?>
    <tr bgcolor="<?php echo alter_color($counter, "#eee", "#FFF"); ?>">
      ...
      <td>
  <?php           
	$showGoToPersonne = "detail_person.php?recordID=". $row_rsShowPersonInactiv['personne_id'];
/*  if (isset($_SERVER['QUERY_STRING'])) {
    $showGoToPersonne .= (strpos($showGoToPersonne, '?')) ? "&" : "?";
    $showGoToPersonne .= $_SERVER['QUERY_STRING'];
  }*/
?>
        <a href="#" onclick="<?php popup($showGoToPersonne, "700", "400"); ?>">RIB</a>&nbsp;</td>
      <td><?php if (isset($row_rsShowPersonInactiv['personne_matricule']) && $row_rsShowPersonInactiv['personne_matricule'] != "XXXXXX-X") {echo $row_rsShowPersonInactiv['personne_matricule']; } else {echo 'Absent'; } ?></td>
      <td><?php echo strtoupper(htmlentities($row_rsShowPersonInactiv['personne_nom'])); ?></td>
      ...
      <td align="center"><?php if (isset($row_rsShowPersonInactiv['display']) && $row_rsShowPersonInactiv['display'] == '1') { ?>
        <a href="change_etat_index.php?recordID=<?php echo $row_rsShowPersonInactiv['personne_id']; ?>&amp;&amp;map=personnes&amp;&amp;mapid=personne_id&amp;&amp;action=desactive&menuID=<?php echo $_GET['menuID']; ?>"><img src="img/s_okay.png" alt="" width="16" height="16" align="absmiddle"/></a>
        <?php } else { ?>
        <a href="change_etat_index.php?recordID=<?php echo $row_rsShowPersonInactiv['personne_id']; ?>&amp;&amp;map=personnes&amp;&amp;mapid=personne_id&amp;&amp;action=active&menuID=<?php echo $_GET['menuID']; ?>"><img src="img/s_cancel.png" alt="" width="16" height="16" align="absmiddle"/></a>
      <?php } ?></td>
    <?php if (isset($_GET['action']) && $_GET['action']=='upd') { ?>
    <td><a href="maj_personnes.php?recordID=<?php echo $row_rsShowPersonInactiv['personne_id']; ?>&amp;&amp;map=personnes"><img src="img/b_edit.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
      <?php } ?>
      <?php if (isset($_GET['action']) && $_GET['action']=='del') { ?>
  <td><a href="delete.php?recordID=<?php echo $row_rsShowPersonInactiv['personne_id']; ?>&amp;&amp;map=personnes&amp;&amp;mapid=personne_id"><img src="img/b_drop.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
  <?php } ?>
  <td><a href="detail_personnes.php"><img src="img/b_view.png" alt="" width="16" height="16" align="absmiddle"/></a></td>
</tr>
