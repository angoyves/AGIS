...
AND commissions.membre_insert = '0'
AND localite_lib LIKE %s ORDER BY commissions.dateCreation ASC", GetSQLValueString("%" . $colname_rsCommissions . "%", "text"));
} else {
$query_rsCommissions = sprintf("SELECT commission_id, natures.lib_nature, type_commission_lib, localite_lib, commission_lib
FROM natures, type_commissions, localites, commissions
WHERE commissions.nature_id = natures.nature_id
...
<table border="1" align="center" class="std">
  ...
  <?php do { ?>
  	<?php if (isset($_GET['page']) && $_GET['page']=='rep'){
		$linkPage = 'new_representant.php'; 
		} else {
		$linkPage = 'new_membres.php';
		}
	?>
    <tr>
      <td nowrap="nowrap"><a href="" onclick="window.opener.location.href='<?php echo $linkPage ?>?comID=<?php echo $row_rsCommissions['commission_id']; ?>';self.close();"> <img src="images/img/b_import.png" width="16" height="16" alt="<?php echo $row_rsCommissions['commission_id']; ?>" /></a></td>
      <td nowrap="nowrap">&nbsp; <?php echo $row_rsCommissions['commission_lib']; ?></td>
      <td nowrap="nowrap"><?php echo $row_rsCommissions['type_commission_lib']; ?>&nbsp; </td>
      <td nowrap="nowrap"><?php echo $row_rsCommissions['localite_lib']; ?>&nbsp</td>
    </tr>
