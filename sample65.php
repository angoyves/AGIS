...
if (empty($_POST['user_password'])){
 $error_msg = "Saisir un mot de passe valide...";
} elseif ($_POST['user_password2']!=$_POST['user_password']){
 $error_msg = "Les mots de passe ne coincident pas";
} else {
	$updateSQL = sprintf("UPDATE users SET user_password=%s, dateUpdate=%s WHERE user_id=%s",
...
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_UpdUserPass = "SELECT * FROM users";
$UpdUserPass = mysql_query($query_UpdUserPass, $MyFileConnect) or die(mysql_error());
$row_UpdUserPass = mysql_fetch_assoc($UpdUserPass);
$colname_UpdUserPass = "-1";
if (isset($_GET['uID'])) {
  $colname_UpdUserPass = $_GET['uID'];
}
mysql_select_db($database_MyFileConnect, $MyFileConnect);
$query_UpdUserPass = sprintf("SELECT * FROM users WHERE user_id = %s", GetSQLValueString($colname_UpdUserPass, "int"));
$UpdUserPass = mysql_query($query_UpdUserPass, $MyFileConnect) or die(mysql_error());
$row_UpdUserPass = mysql_fetch_assoc($UpdUserPass);
$totalRows_UpdUserPass = mysql_num_rows($UpdUserPass);
?>
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  ...
  <table align="center">
    ...
    <tr valign="baseline">
      <td colspan="2" align="center" nowrap="nowrap" class="error"><?php echo $error_msg; ?>&nbsp;</td>
      </tr>
    <tr valign="baseline">
    ...
    <tr valign="baseline">
      <th align="right" nowrap="nowrap">Confirmer le nouveau mot de passe</th>
      <td><input type="password" name="user_password2" value="" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">&nbsp;</td>
  ...
  <input type="hidden" name="user_id" value="<?php echo htmlentities($row_UpdUserPass['user_id'], ENT_COMPAT, 'utf-8'); ?>" size="32" />
  <input type="hidden" name="MM_update" value="form1" />
  <input type="hidden" name="user_id" value="<?php echo $row_UpdUserPass['user_id']; ?>" />
</form>
<p>&nbsp;</p>
<!-- InstanceEndEditable -->
    <!--div class="bloc express">
